...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if (Schema::hasTable($rolesTable)) {
            Schema::table($rolesTable, function (Blueprint $table) use ($rolesTable): void {
                if (! Schema::hasColumn($rolesTable, 'company_access_restricted')) {
                    $table->boolean('company_access_restricted')->default(false)->index();
                }

                if (! Schema::hasColumn($rolesTable, 'branch_access_restricted')) {
                    $table->boolean('branch_access_restricted')->default(false)->index();
                }

                if (! Schema::hasColumn($rolesTable, 'financial_period_access_restricted')) {
                    $table->boolean('financial_period_access_restricted')->default(false)->index();
                }
            });
        }

        if (Schema::hasTable($rolesTable) && Schema::hasTable('companies') && ! Schema::hasTable('role_company_access')) {
        ...
        if (Schema::hasTable($rolesTable) && Schema::hasTable('branches') && ! Schema::hasTable('role_branch_access')) {
        ...
        if (Schema::hasTable($rolesTable) && Schema::hasTable('financial_periods') && ! Schema::hasTable('role_financial_period_access')) {
