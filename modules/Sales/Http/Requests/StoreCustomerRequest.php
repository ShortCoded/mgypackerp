...
class StoreCustomerRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            'doc_number' => ['nullable', 'integer', 'min:1', $this->uniqueActiveCustomerRule('doc_number')],
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator): void {
            ...
            if ($this->filled('account_group_doc_num') && ! $parentAccount) {
                $validator->errors()->add('account_group_doc_num', __('customers.messages.customer_group_unavailable'));
            }

            if ($this->filled('account_group_doc_num') && $parentAccount && (! $parentAccount->is_group || $parentAccount->is_postable)) {
                $validator->errors()->add('account_group_doc_num', __('customers.messages.customer_group_must_be_group'));
            }

            if ($this->filled('account_group_doc_num') && $parentAccount && $parentAccount->is_group && ! $accounts->isSelectableGroup(BusinessPartnerAccountService::Customer, $parentAccount)) {
                $validator->errors()->add('account_group_doc_num', __('customers.messages.customer_group_unavailable'));
            }

            if ($parentAccount && $this->hasDuplicateLinkedAccount($parentAccount)) {
                $validator->errors()->add('account_group_doc_num', __('customers.messages.linked_account_duplicate'));
            }
    ...
    {
        return [
            'doc_number' => __('customers.attributes.doc_number'),
            'name' => __('customers.attributes.name'),
            'account_group_doc_num' => __('customers.attributes.account_group'),
            'status' => __('customers.attributes.status'),
            'phone' => __('customers.attributes.phone'),
            'mobile' => __('customers.attributes.mobile'),
            'email' => __('customers.attributes.email'),
            'tax_number' => __('customers.attributes.tax_number'),
            'commercial_register' => __('customers.attributes.commercial_register'),
            'national_id' => __('customers.attributes.national_id'),
            'contact_person' => __('customers.attributes.contact_person'),
            'address' => __('customers.attributes.address'),
            ...
            'area_doc_num' => __('customers.attributes.area'),
            'credit_limits' => __('customers.attributes.credit_limits'),
            'credit_limits.*.currency_doc_num' => __('customers.attributes.currency'),
            'credit_limits.*.credit_limit' => __('customers.attributes.credit_limit'),
            'credit_limits.*.notes' => __('customers.attributes.notes'),
    ...
    {
        return [
            'doc_number.unique' => __('customers.messages.doc_number_unique'),
        ];
    }
    ...
    private function resolvedParentAccount(): ?Account
    {
        if (! $this->filled('account_group_doc_num')) {
            try {
                return app(BusinessPartnerAccountService::class)->rootAccount(BusinessPartnerAccountService::Customer);
    ...
    private function uniqueActiveCustomerRule(string $column): Unique
    {
        $companyId = Arr::get(app(OperatingContextService::class)->snapshot($this), 'company_id');
    ...
    private function validateCreditLimits(Validator $validator): void
    {
        ...
        foreach ($this->input('credit_limits', []) as $index => $row) {
            ...
            if ($currencyDocNum === '') {
                $validator->errors()->add("credit_limits.{$index}.currency_doc_num", __('validation.required', ['attribute' => __('customers.attributes.currency')]));
            ...
            if (! $currency instanceof Currency) {
                $validator->errors()->add("credit_limits.{$index}.currency_doc_num", __('validation.exists', ['attribute' => __('customers.attributes.currency')]));
