...
return new class extends Migration
{
    ...
    public function down(): void
    {
        if (in_array(DB::getDriverName(), ['pgsql', 'sqlite'], true)) {
            foreach ([
                'customers_company_doc_number_unique_active',
                'customers_company_doc_num_unique_active',
                'customers_company_account_id_unique_active',
            ] as $index) {
    ...
    private function createActiveUniqueIndexes(): void
    {
        ...
        $table = $grammar->wrapTable('customers');
        $deletedAt = $grammar->wrap('deleted_at');
        $companyId = $grammar->wrap('company_id');

        foreach (['doc_number', 'doc_num', 'account_id'] as $column) {
            $wrappedColumn = $grammar->wrap($column);
            DB::statement('CREATE UNIQUE INDEX IF NOT EXISTS '.$grammar->wrap("customers_company_{$column}_unique_active")." ON {$table} ({$companyId}, {$wrappedColumn}) WHERE {$deletedAt} IS NULL AND {$wrappedColumn} IS NOT NULL");
        }
    }
};
