...
class SupplierService
{
    public function __construct(
        ...
        private readonly SupplierAccountingSyncService $accountingSync,
    ...
    public function create(array $data): array
    {
        return DB::transaction(function () use ($data): array {
            ...
            $linkedAccount = $this->accounts->createOrUpdateLinkedAccount(BusinessPartnerAccountService::Supplier, null, $parentAccount, $data)['account'];
            $record = Supplier::query()->create([
    ...
    public function update(Supplier $record, array $data): array
    {
        return DB::transaction(function () use ($record, $data): array {
            ...
            $chartResult = $this->syncsLinkedAccount($record, $data, $parentAccount)
                ? $this->accounts->createOrUpdateLinkedAccount(BusinessPartnerAccountService::Supplier, $record->account, $parentAccount, $data)
    ...
    public function delete(Supplier $record): void
    {
        DB::transaction(function () use ($record): void {
            $this->accountingSync->softDeleteLinkedAccountForSupplier($record);
    ...
    public function bulkDelete(array $docNums): int
    {
        return DB::transaction(function () use ($docNums): int {
            ...
            foreach (Supplier::query()->forCompany($this->companies->requireCompanyId())->whereIn('doc_num', $docNums)->get() as $record) {
                $this->delete($record);
                $deleted++;
            }
    ...
    public function restore(Supplier $record): Supplier
    {
        return DB::transaction(function () use ($record): Supplier {
            if (Supplier::query()->forCompany((int) $record->company_id)->where('account_id', $record->account_id)->whereKeyNot($record->getKey())->exists()) {
                throw new DomainException(__('suppliers.messages.restore_conflict'));
            }

            $this->accountingSync->restoreLinkedAccountForSupplier($record);
    ...
    private function document(array $data, int $companyId): array
    {
        return array_key_exists('doc_number', $data) && $data['doc_number']
            ? ['doc_number' => (int) $data['doc_number'], 'doc_num' => $this->documents->format('suppliers', (int) $data['doc_number'])]
            : $this->documents->nextForCompany('suppliers', Supplier::class, $companyId);
    }
    ...
    {
        return $this->accounts->parentAccount(BusinessPartnerAccountService::Supplier, $data['account_group_doc_num'] ?? null);
    }
    ...
    private function assertLinkedAccountCanMove(?Account $linkedAccount, Account $parentAccount): void
    {
        ...
        if ($linkedAccount->children()->exists()) {
            throw new DomainException(__('suppliers.messages.account_move_blocked_children'));
        }
    }

    private function syncsLinkedAccount(Supplier $record, array $data, Account $parentAccount): bool
    {
        if (! $record->account instanceof Account || ! $this->accounts->isManagedLinkedAccount(BusinessPartnerAccountService::Supplier, $record->account)) {
            return true;
        }
     ...
     */
    private function syncCreditLimits(Supplier $record, array $rows, int $companyId): void
    {
        ...
        foreach ($incoming as $row) {
            ...
            /** @var SupplierCreditLimit|null $limit */
            $limit = $existing->get($row['currency_id']);

            if ($limit instanceof SupplierCreditLimit) {
                if ($limit->trashed()) {
                    $limit->restore();
                }
        ...
        $record->creditLimits()
            ...
            ->get()
            ->each(function (SupplierCreditLimit $limit): void {
     ...
     * @param  list<array<string, mixed>>  $rows
     */
    private function creditLimitsChanged(Supplier $record, array $rows): bool
    {
        ...
        $existing = $record->creditLimits()
            ...
            ->map(fn (SupplierCreditLimit $limit): array => [
