...
class UpdateHrFoundationRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $record = $this->record();
        $nameUnique = Rule::unique($definition->table, 'name')
    ...
    /**
     * @return list<callable(Validator): mixed>
     */
    ...
    private function record(): ?HrFoundationModel
    {
        foreach ($this->route()?->parameters() ?? [] as $record) {
            if ($record instanceof HrFoundationModel) {
                return $record;
            }
        }
    ...
    private function rulesForField(array $field): array
    {
        ...
        if (($field['unique'] ?? false) === true) {
            $unique = Rule::unique($this->definition()->table, (string) ($field['column'] ?? $field['name']))
    ...
    private function validateDocumentNumber(Validator $validator): void
    {
        ...
        if (! $this->canControlDocumentNumber()
            || ! $record instanceof HrFoundationModel
