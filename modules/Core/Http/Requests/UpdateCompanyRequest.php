...
class UpdateCompanyRequest extends FormRequest
{
    ...
    {
        return (bool) $this->user()?->can('companies.edit');
    }
    ...
    public function rules(): array
    {
        ...
        $rules = [
            ...
            'name' => ['required', 'string', 'max:255', Rule::unique('companies', 'name')->ignore($company?->id)->withoutTrashed()],
            ...
            'tax_card_number' => ['nullable', 'string', 'max:100', Rule::unique('companies', 'tax_card_number')->ignore($company?->id)->withoutTrashed()],
            'tax_file_number' => ['nullable', 'string', 'max:100'],
            'tax_office' => ['nullable', 'string', 'max:255'],
            'vat_registration_number' => ['nullable', 'string', 'max:100', Rule::unique('companies', 'vat_registration_number')->ignore($company?->id)->withoutTrashed()],
            ...
            'email' => ['nullable', 'email:rfc', 'max:255', Rule::unique('companies', 'email')->ignore($company?->id)->withoutTrashed()],
        ...
        if ($this->canControlDocumentNumber()) {
            $rules['doc_number'] = [
                'nullable',
                'regex:/^\d+$/',
                Rule::unique('companies', 'doc_number')->ignore($company?->id)->withoutTrashed(),
            ];
        }
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator): void {
            ...
            if (Company::query()->where('doc_num', $docNum)->whereKeyNot($company->getKey())->exists()) {
        ...
        $validator->after(function (Validator $validator): void {
            $company = $this->route('company');
            $companyId = $company instanceof Company ? (int) $company->getKey() : null;
    ...
    public function validated($key = null, $default = null): mixed
    {
        ...
        if (array_key_exists('remove_logo', $data)) {
            $data['remove_logo'] = $this->boolean('remove_logo');
        }

        if (array_key_exists('remove_favicon', $data)) {
            $data['remove_favicon'] = $this->boolean('remove_favicon');
        }
