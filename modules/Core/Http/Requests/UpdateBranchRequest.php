...
class UpdateBranchRequest extends FormRequest
{
    public function authorize(): bool
    {
        ...
        return $user instanceof User
            ...
            && app(CompanyAccessService::class)->canAccessCompany($user, $branch->company);
    }
    ...
    public function rules(): array
    {
        ...
        if ($this->canControlDocumentNumber()) {
            $rules['doc_number'] = [
                'nullable',
                'regex:/^\d+$/',
                Rule::unique('branches', 'doc_number')->ignore($branch?->getKey())->withoutTrashed(),
            ];
        }
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator): void {
            ...
            if ($branch instanceof Branch && $company instanceof Company && Branch::query()
            ...
            if (Branch::query()->where('doc_num', $docNum)->whereKeyNot($branch->getKey())->exists()) {
        ...
        $validator->after(function (Validator $validator): void {
            ...
            if ($branch instanceof Branch) {
                $this->validateExistingStationHallConflicts($validator, $branch);
            }
        });
        ...
        $validator->after(function (Validator $validator): void {
            ...
            if ($branch instanceof Branch) {
                $this->validateExistingBranchStoreConflicts($validator, $branch);
            }
        });
    }
    ...
    private function validateExistingStationHallConflicts(Validator $validator, Branch $branch): void
    {
        $existing = $branch->halls()->get(['id', 'public_uuid', 'name'])->keyBy('public_uuid');
        $existingByName = $existing->keyBy(fn (BranchHall $hall): string => $this->normalizedName($hall->name));

        foreach ($this->stationHallPayload() as $index => $hall) {
            $key = $hall['key'] ?? null;
            $name = $this->normalizedName($hall['name']);

            if ($key !== null && ! $existing->has($key)) {
                $validator->errors()->add("station_halls.{$index}.key", __('branches.validation.detail_key_invalid'));

                continue;
            }

            $conflictingHall = $existingByName->get($name);

            if ($conflictingHall instanceof BranchHall
                && $key !== null
                && (string) $conflictingHall->public_uuid !== $key) {
                $validator->errors()->add("station_halls.{$index}.name", __('branches.validation.station_halls_distinct'));
            }
        }
    }
    ...
    private function validateExistingBranchStoreConflicts(Validator $validator, Branch $branch): void
    {
        $existing = $branch->stores()->get(['id', 'public_uuid', 'name'])->keyBy('public_uuid');
        $existingByName = $existing->keyBy(fn (BranchStore $store): string => $this->normalizedName($store->name));

        foreach ($this->branchStorePayload() as $index => $store) {
            $key = $store['key'] ?? null;
            $name = $this->normalizedName($store['name']);

            if ($key !== null && ! $existing->has($key)) {
                $validator->errors()->add("branch_stores.{$index}.key", __('branches.validation.detail_key_invalid'));

                continue;
            }

            $conflictingStore = $existingByName->get($name);

            if ($conflictingStore instanceof BranchStore
                && $key !== null
                && (string) $conflictingStore->public_uuid !== $key) {
                $validator->errors()->add("branch_stores.{$index}.name", __('branches.validation.branch_stores_distinct'));
            }
        }
    }
}
