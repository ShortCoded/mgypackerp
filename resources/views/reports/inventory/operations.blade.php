...
<table><thead><tr><th>{{ __('Receipt date') }}</th><th>{{ __('Age / bucket') }}</th><th>{{ __('Receipt source') }}</th><th>{{ __('Store') }}</th><th>{{ __('Product') }}</th><th>{{ __('Status / batch') }}</th><th class="number">{{ __('Remaining quantity') }}</th>@if($canViewFinancial)<th class="number">{{ __('Remaining value') }}</th>@endif</tr></thead><tbody>@foreach($agingLayers as $layer)<tr><td>{{ $layer->original_receipt_date?->toDateString() }}</td><td>{{ $layer->age_days }} / {{ $layer->age_bucket }}</td><td>{{ $layer->source_doc_num }}</td><td>{{ $layer->branchStore?->name }}</td><td>{{ $layer->product?->doc_num }} — {{ $layer->product?->name }}</td><td>{{ $layer->stock_status }} / {{ $layer->batch_lot }}</td><td class="number">{{ $layer->remaining_quantity }}</td>@if($canViewFinancial)<td class="number">{{ $layer->remaining_value }}</td>@endif</tr>@endforeach<tr class="total"><td colspan="6">{{ __('Total') }}</td><td class="number">{{ $reportTotals['aging_quantity'] }}</td>@if($canViewFinancial)<td class="number">{{ $reportTotals['aging_value'] }}</td>@endif</tr></tbody></table>
