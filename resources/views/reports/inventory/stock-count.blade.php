...
@section('report')
    ...
    <table class="report-table" style="margin-bottom:9px;"><tbody>
        <tr><th>{{ __('Date') }}</th><td>{{ $dates->formatDate($record->count_date, '') }}</td><th>{{ __('Status') }}</th><td>{{ __('inventory.movements.statuses.'.$record->status) }}</td></tr>
