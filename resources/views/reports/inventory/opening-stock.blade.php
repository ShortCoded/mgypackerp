...
@section('report')
    ...
    <table class="report-table" style="margin-bottom:9px;"><tbody>
        <tr><th>{{ __('Date') }}</th><td>{{ $dates->formatDate($record->document_date, '') }}</td><th>{{ __('Status') }}</th><td>{{ str($record->status)->title() }}</td></tr>
    ...
    <table class="report-table opening-lines"><thead><tr><th>#</th><th>{{ __('Product') }}</th><th>{{ __('Quantity') }}</th><th>{{ __('Status') }}</th><th>{{ __('Batch') }}</th><th>{{ __('Location') }}</th><th>{{ __('Notes') }}</th></tr></thead><tbody>
        @foreach($record->lines as $line)<tr><td>{{ $line->line_no }}</td><td>{{ $line->product?->doc_num }} / {{ $line->product?->name }}</td><td dir="ltr">{{ $numbers->format($line->quantity) }}</td><td>{{ str($line->stock_status ?: 'available')->replace('_', ' ')->title() }}</td><td dir="ltr">{{ $line->batch_lot ?: '—' }}</td><td>{{ $line->warehouseLocation?->code ?: '—' }}</td><td>{{ $line->notes }}</td></tr>@endforeach
    </tbody></table>
