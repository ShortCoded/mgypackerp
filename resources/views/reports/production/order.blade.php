...
@section('report')
    ...
    <table class="report-table" style="margin-bottom:9px"><tbody>
        ...
        @if($record->salesOrder?->salesEmployee)<tr><th>{{ __('Sales representative') }}</th><td colspan="3">{{ $record->salesOrder->salesEmployee->doc_num }} / {{ $record->salesOrder->salesEmployee->full_name ?: $record->salesOrder->salesEmployee->name }}</td></tr>@endif
    </tbody></table>
