...
<table class="report-table"><thead><tr><th>{{ __('Invoice') }}</th><th>{{ __('Customer') }}</th><th>{{ __('Date') }}</th><th>{{ __('Sales Order') }}</th><th>{{ __('Delivery Notes') }}</th><th>{{ __('Net Sales') }}</th><th>{{ __('Collected') }}</th><th>{{ __('Outstanding') }}</th></tr></thead><tbody>@foreach($salesLedger as $invoice)<tr><td>{{ $invoice->doc_num }}</td><td>{{ $invoice->customer?->name }}</td><td>{{ $dateValue($invoice->invoice_date) }}</td><td>{{ $invoice->order?->doc_num ?: '—' }}</td><td>{{ $invoice->deliveries->pluck('doc_num')->join(' / ') ?: '—' }}</td><td>{{ $numbers->format(bcsub((string) $invoice->total_amount, (string) ($invoice->returns_amount ?? 0), 4)) }}</td><td>{{ $numbers->format($invoice->paid_amount) }}</td><td>{{ $numbers->format($invoice->remaining_amount) }}</td></tr>@endforeach</tbody></table>
@endif
