...
@section('report')
    ...
    <div class="report-filter-summary">{{ __('Currency') }}: {{ $reportCurrency?->code }} @if($from || $to) · {{ $from?->toDateString() }} — {{ $to?->toDateString() }} @endif</div>
    <h3>{{ __('Quotation Status / History') }}</h3>
    <table class="report-table"><thead><tr><th>{{ __('Quotation') }}</th><th>{{ __('Customer') }}</th><th>{{ __('Date') }}</th><th>{{ __('Status') }}</th><th>{{ __('Total') }}</th></tr></thead><tbody>@foreach($quotations as $quotation)<tr><td>{{ $quotation->doc_num }}</td><td>{{ $quotation->customer?->name }}</td><td>{{ $quotation->quotation_date?->toDateString() }}</td><td>{{ __(str($quotation->status)->replace('_', ' ')->title()->toString()) }}</td><td>{{ $numbers->format($quotation->currentRevision?->total) }}</td></tr>@endforeach</tbody></table>
    <h3>{{ __('Ordered vs Reserved vs Produced vs Delivered') }}</h3>
    <table class="report-table"><thead><tr><th>{{ __('Order') }}</th><th>{{ __('Customer') }}</th><th>{{ __('Status') }}</th><th>{{ __('Ordered') }}</th><th>{{ __('Reserved') }}</th><th>{{ __('Produced') }}</th><th>{{ __('Delivered') }}</th></tr></thead><tbody>@foreach($openOrders as $order)<tr><td>{{ $order->doc_num }}</td><td>{{ $order->customer?->name }}</td><td>{{ __(str($order->status)->replace('_', ' ')->title()->toString()) }}</td><td>{{ $numbers->format($order->ordered_quantity) }}</td><td>{{ $numbers->format($order->reserved_quantity) }}</td><td>{{ $numbers->format($order->produced_quantity) }}</td><td>{{ $numbers->format($order->delivered_quantity) }}</td></tr>@endforeach</tbody></table>
