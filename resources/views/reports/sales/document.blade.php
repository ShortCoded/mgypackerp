...
@section('report')
            ...
            <tbody>
                @foreach($lines as $line)
                    ...
                    <tr>
                        ...
                        @if(in_array($kind, ['sales_return', 'quality_disposition'], true))<td>{{ $line->quality_disposition ? __(str($line->quality_disposition)->replace('_', ' ')->title()->toString()) : __('Pending inspection') }}<br>{{ __('Saleable') }}: {{ $numbers->format($line->saleable_quantity) }} · {{ __('Quarantine') }}: {{ $numbers->format($line->quarantine_quantity) }} · {{ __('Rework') }}: {{ $numbers->format($line->rework_quantity) }} · {{ __('Scrap') }}: {{ $numbers->format($line->scrap_quantity) }}</td>@endif
                    </tr>
                @endforeach
            </tbody>
        </table>
    ...
    @if($showPrices && isset($record->total_amount)) @include('reports.partials.amount-in-words') @endif
