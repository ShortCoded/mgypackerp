...
@section('fixed_asset_content')
    ...
    <div class="fa-pdf-section">
        ...
        <table class="fa-pdf-values-table">
            ...
            @if($disposal->settlement_path === \Modules\FixedAssets\Models\FixedAssetDisposal::SettlementCustomerInvoice)<tr><th>{{ __('VAT') }}</th><td class="fa-pdf-number">{{ $numbers->format($disposal->tax_amount) }}</td><th>{{ __('Gross customer receivable') }}</th><td class="fa-pdf-number">{{ $numbers->format($disposal->gross_proceeds) }}</td></tr><tr><th>{{ __('Customer Invoice') }}</th><td>{{ $disposal->customerInvoice?->doc_num }}</td><th>{{ __('Disposal clearing / gain-loss journal') }}</th><td>{{ $disposal->gainLossJournalEntry?->doc_num }}</td></tr>@endif
            @if ($showProceeds)
