@foreach ($items as $item)
    ...
    @if ($hasChildren)
        <li class="nav-item dropdown erp-top-nav-root">
            <a class="nav-link dropdown-toggle {{ $item['active'] ? 'active' : '' }}" id="{{ $menuId }}" href="#" role="button" data-erp-menu-toggle aria-haspopup="true" aria-controls="{{ $submenuId }}" aria-expanded="false">
                {{ $item['text'] }}
            </a>
            <div class="dropdown-menu dropdown-caret dropdown-menu-card erp-top-nav-menu mt-0" id="{{ $submenuId }}" aria-labelledby="{{ $menuId }}">
