    ...
    <div class="card mb-3">
        ...
        <div class="card-body">
            <form method="GET" action="{{ route('admin.finance.cashbox-count.index') }}" class="row g-3">
                <div class="col-12 col-md-4"><label class="form-label" for="cashbox-count-as-of">{{ __('cashbox_count.filters.as_of_date') }}</label><x-forms.input class="form-control" id="cashbox-count-as-of" name="as_of_date" type="date" value="{{ $filters['as_of_date'] ?? '' }}" /></div>
        ...
        <div class="table-responsive"><table class="table table-sm table-striped align-middle mb-0">
            ...
            <tbody>@forelse($report['rows'] as $index => $row)
                ...
                <tr data-cashbox-count-row data-book-balance="{{ $row['balance'] }}">
                    ...
                    <td><form id="cashbox-count-form-{{ $index }}" method="POST" action="{{ route('admin.finance.cashbox-count.store') }}">@csrf<input type="hidden" name="count_date" value="{{ $filters['as_of_date'] }}"><input type="hidden" name="cashbox_doc_num" value="{{ $cashbox?->doc_num }}"><input type="hidden" name="currency_doc_num" value="{{ $currency?->doc_num }}"><x-forms.input class="form-control form-control-sm text-end js-cashbox-counted" name="actual_amount" type="number" min="0" step="0.0001" required /></form></td>
