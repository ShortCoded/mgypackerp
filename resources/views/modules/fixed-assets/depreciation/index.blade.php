...
@section('content')
    ...
    <div class="card mb-3">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.fixed-assets.depreciation.preview') }}" class="row g-3 align-items-end">@csrf
                <div class="col-md-4 col-xl-3"><label class="form-label" for="depreciation-financial_period_doc_num">{{ __('fixed_assets.lifecycle.financial_period') }}</label><x-forms.select id="depreciation-financial_period_doc_num" class="form-select js-select2-ajax" name="financial_period_doc_num" data-url="{{ route('admin.select2.financial-periods') }}" data-placeholder="{{ __('fixed_assets.placeholders.financial_period') }}" required>@if($financialPeriod)<option value="{{ $financialPeriod->doc_num }}" selected>{{ $financialPeriod->doc_num }} / {{ $financialPeriod->name }}</option>@endif</x-forms.select></div>
                <div class="col-md-4 col-xl-3"><label class="form-label" for="depreciation-posting_date">{{ __('fixed_assets.usability.depreciation_month_end') }}</label><x-forms.date-input id="depreciation-posting_date" class="form-control js-date-picker" name="posting_date" value="{{ $dates->formatDate($dates->normalizeForStorage(old('posting_date', $postingDate)), '') }}" data-date-format="{{ $dates->jsDateFormat() }}" data-locale="{{ app()->getLocale() }}" required /><div class="form-text">{{ __('fixed_assets.usability.depreciation_month_end_help') }}</div></div>
                <div class="col-md-4 col-xl-3"><label class="form-label" for="depreciation-branch_doc_num">{{ __('fixed_assets.attributes.branch') }}</label><x-forms.select id="depreciation-branch_doc_num" class="form-select js-select2-ajax" name="branch_doc_num" data-url="{{ route('admin.fixed-assets.select2.branches') }}" data-placeholder="{{ __('fixed_assets.placeholders.branch') }}" data-allow-clear="true"></x-forms.select></div>
                <div class="col-md-4 col-xl-3"><label class="form-label" for="depreciation-asset_group_account_doc_num">{{ __('fixed_assets.attributes.asset_group_account') }}</label><x-forms.select id="depreciation-asset_group_account_doc_num" class="form-select js-select2-ajax" name="asset_group_account_doc_num" data-url="{{ route('admin.fixed-assets.select2.asset-categories') }}" data-placeholder="{{ __('fixed_assets.placeholders.asset_category') }}" data-allow-clear="true"></x-forms.select></div>
                <div class="col-md-4 col-xl-3"><label class="form-label" for="depreciation-cost_center_doc_num">{{ __('fixed_assets.attributes.cost_center') }}</label><x-forms.select id="depreciation-cost_center_doc_num" class="form-select js-select2-ajax" name="cost_center_doc_num" data-url="{{ route('admin.fixed-assets.select2.cost-centers') }}" data-placeholder="{{ __('fixed_assets.placeholders.cost_center') }}" data-allow-clear="true"></x-forms.select></div>
                <div class="col-md-8 col-xl-6"><label class="form-label">{{ __('fixed_assets.reports.columns.asset') }}</label><x-forms.select class="form-select js-select2-ajax" name="asset_doc_nums[]" multiple data-url="{{ route('admin.fixed-assets.select2.assets') }}" data-placeholder="{{ __('fixed_assets.placeholders.asset') }}" data-allow-clear="true">@foreach($selectedAssets as $selectedAsset)<option value="{{ $selectedAsset->doc_num }}" selected>{{ $selectedAsset->doc_num }} / {{ $selectedAsset->asset_name }}</option>@endforeach</x-forms.select></div>
    ...
    @if($preview)
        <form method="POST" action="{{ route('admin.fixed-assets.depreciation.post') }}" class="js-depreciation-post-form" data-preview-url="{{ route('admin.fixed-assets.depreciation.preview') }}" data-confirm-title="{{ __('fixed_assets.lifecycle.confirm_depreciation_title') }}" data-confirm-text="{{ __('fixed_assets.lifecycle.confirm_depreciation_text') }}" data-confirm-yes="{{ __('fixed_assets.lifecycle.confirm_depreciation_yes') }}" data-confirm-cancel="{{ __('fixed_assets.js.cancel') }}">@csrf
            <x-forms.input type="hidden" name="financial_period_doc_num" value="{{ $preview['financialPeriod']->doc_num }}" /><x-forms.input type="hidden" name="posting_date" value="{{ $preview['postingDate']->toDateString() }}" />
            @foreach(['branch_doc_num', 'asset_group_account_doc_num', 'cost_center_doc_num'] as $filter)<x-forms.input type="hidden" name="{{ $filter }}" value="{{ request($filter) }}" />@endforeach
        ...
        @if($rows->isNotEmpty())
        <details class="card mb-3 border-{{ $actionable ? 'warning' : 'danger' }}" @if($actionable) open @endif>
            <summary class="card-header bg-{{ $actionable ? 'warning' : 'danger' }}-subtle text-{{ $actionable ? 'warning' : 'danger' }}">{{ __($actionable ? 'fixed_assets.usability.needs_action' : 'fixed_assets.lifecycle.excluded_assets') }} ({{ $rows->count() }})</summary>
            ...
            @foreach($rows as $row)<tr><td><a href="{{ route('admin.fixed-assets.assets.show', $row['asset']) }}">{{ $row['asset']->doc_num }} / {{ $row['asset']->asset_name }}</a></td><td class="{{ $actionable ? 'text-warning' : 'text-danger' }}">{{ $row['reason'] }}</td><td>
                @if($actionable)
                    @if($row['next_date'] ?? null)
                    @php($requiredPeriod = $requiredPeriods[$row['next_date']] ?? null)
                    @if($requiredPeriod && ! $requiredPeriod['is_closed'])
                    <form method="POST" action="{{ route('admin.fixed-assets.depreciation.preview') }}">@csrf<x-forms.input type="hidden" name="financial_period_doc_num" value="{{ $requiredPeriod['doc_num'] }}" /><x-forms.input type="hidden" name="posting_date" value="{{ $row['next_date'] }}" /><x-forms.input type="hidden" name="asset_doc_nums[]" value="{{ $row['asset']->doc_num }}" /><button class="btn btn-falcon-primary btn-sm" type="submit" title="{{ $requiredPeriod['label'] }}">{{ __('fixed_assets.usability.run_required_month', ['period' => $requiredPeriod['month']]) }}</button></form>
