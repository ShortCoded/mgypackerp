...
<form method="POST" action="{{ $editing ? route('admin.sales.price-lists.update', $record) : route('admin.sales.price-lists.store') }}" data-price-list-form data-mode="{{ $mode }}" novalidate>
    ...
    @if($clone && $cloneSourceToken)
        <x-forms.input type="hidden" name="clone_source_token" value="{{ $cloneSourceToken }}" />
        <x-forms.input type="hidden" name="_submission_token" value="{{ $cloneSourceToken }}" />
    @endif
...
<div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
    ...
    <fieldset @disabled($previewOnly)>
        <div class="card mb-3"><div class="card-body"><div class="row g-3">
            ...
            <div class="col-md-3"><label class="form-label" for="price_list_date">{{ __('price_lists.fields.date') }}</label>@if($readOnly)<x-forms.input id="price_list_date" class="form-control" :value="$dates->formatDate($record?->price_list_date, '')" disabled dir="ltr" />@else<x-forms.date-input id="price_list_date" name="price_list_date" :value="old('price_list_date', $record?->price_list_date?->toDateString() ?? now()->toDateString())" required />@endif @error('price_list_date')<div class="text-danger small">{{ $message }}</div>@enderror</div>
            <div class="col-md-3"><label class="form-label" for="customer_doc_num">{{ __('price_lists.fields.customer') }}</label><x-forms.select id="customer_doc_num" name="customer_doc_num" variant="ajax" :url="route('admin.sales.select2.customers')" :placeholder="__('price_lists.general')"><option value=""></option>@foreach($selectedCustomers as $customer)<option value="{{ $customer->doc_num }}" selected>{{ $customer->doc_num }} / {{ $customer->name }}</option>@endforeach</x-forms.select><div class="form-text">{{ __('price_lists.customer_help') }}</div>@error('customer_doc_num')<div class="text-danger small">{{ $message }}</div>@enderror</div>
            <div class="col-md-3"><label class="form-label" for="currency_doc_num">{{ __('price_lists.fields.currency') }}</label><x-forms.select id="currency_doc_num" name="currency_doc_num" variant="ajax" :url="route('admin.select2.currencies')" :allow-clear="false" required>@foreach($currencies as $currency)<option value="{{ $currency->doc_num }}" @selected(old('currency_doc_num', $record?->currency?->doc_num ?? $currencies->first()?->doc_num) === $currency->doc_num)>{{ $currency->code }} — {{ $currency->name }}</option>@endforeach</x-forms.select>@error('currency_doc_num')<div class="text-danger small">{{ $message }}</div>@enderror</div>
            <div class="col-md-3"><label class="form-label" for="valid_from">{{ __('price_lists.fields.valid_from') }}</label>@if($readOnly)<x-forms.input id="valid_from" class="form-control" :value="$dates->formatDate($record?->valid_from, '')" disabled dir="ltr" />@else<x-forms.date-input id="valid_from" name="valid_from" :value="old('valid_from', $record?->valid_from?->toDateString() ?? now()->toDateString())" required />@endif @error('valid_from')<div class="text-danger small">{{ $message }}</div>@enderror</div>
            <div class="col-md-3"><label class="form-label" for="valid_until">{{ __('price_lists.fields.valid_until') }}</label>@if($readOnly)<x-forms.input id="valid_until" class="form-control" :value="$dates->formatDate($record?->valid_until, '')" disabled dir="ltr" />@else<x-forms.date-input id="valid_until" name="valid_until" :value="old('valid_until', $record?->valid_until?->toDateString())" />@endif<div class="form-text">{{ __('price_lists.valid_until_help') }}</div>@error('valid_until')<div class="text-danger small">{{ $message }}</div>@enderror</div>
        ...
        <div class="card mb-3"><div class="card-header d-flex flex-wrap justify-content-between align-items-center gap-2"><div><h6 class="mb-0">{{ __('price_lists.fields.items') }}</h6><small class="text-600">{{ __('price_lists.base_unit_help') }}</small></div>@unless($previewOnly)<button class="btn btn-falcon-primary btn-sm" type="button" data-price-list-add data-shortcut-action="line.add" title="{{ __('common.shortcuts.add_line') }}" data-bs-title="{{ __('common.shortcuts.add_line') }}"><span class="fas fa-plus me-1"></span>{{ __('price_lists.add_item') }}</button>@endunless</div>
            <div class="table-responsive"><table class="table table-sm table-bordered align-middle mb-0"><thead class="bg-100"><tr><th>#</th><th style="min-width:20rem">{{ __('price_lists.fields.product') }}</th><th>{{ __('price_lists.fields.price') }}</th><th>{{ __('price_lists.fields.discount_type') }}</th><th>{{ __('price_lists.fields.discount_value') }}</th><th></th></tr></thead><tbody data-price-list-lines>
                @foreach($lines as $index => $line)@include('modules.sales.price-lists.line', ['index' => $index, 'line' => $line, 'readOnly' => $previewOnly, 'numbers' => $numbers])@endforeach
            </tbody></table></div>
            @unless($previewOnly)<div class="card-footer d-flex justify-content-end"><button class="btn btn-falcon-primary btn-sm" type="button" data-price-list-add data-shortcut-action="line.add" title="{{ __('common.shortcuts.add_line') }}" data-bs-title="{{ __('common.shortcuts.add_line') }}"><span class="fas fa-plus me-1"></span>{{ __('price_lists.add_item') }}</button></div>@endunless
        </div>
        @if($record && !$clone)
            <div class="card mb-3">
                <div class="card-header"><h6 class="mb-0">{{ __('common.sections.audit_information') }}</h6></div>
                <div class="card-body">
                    <x-audit-fields-row :metadata="$metadata" :show-deleted="$record->trashed()" :show-restored="!$record->trashed() && ($record->restored_at || $record->restored_by)" />
                </div>
            </div>
        @endif
    </fieldset>
