...
@php
    $resource = 'customers';
    $routePrefix = 'admin.sales.customers';
    $title = __('customers.title');
    $columns = ['doc_num', 'name', 'account', 'phone', 'mobile', 'email', 'tax_number', 'status', 'created_by', 'created_at', 'updated_by', 'updated_at'];
@endphp

@section('title', $title)

@section('content')
    @can('customers.document_number_settings.update')
        <div class="card mb-3">
            <div class="card-header py-2">
                <button class="btn btn-link text-decoration-none p-0 w-100 text-start d-flex align-items-center justify-content-between"
                    ...
                    data-bs-target="#customers-document-number-settings"
                    aria-expanded="false"
                    aria-controls="customers-document-number-settings">
                    <span class="fw-semibold">{{ __('common.document_number_settings.title') }}</span>
                    <span class="fas fa-chevron-down fs-11"></span>
                </button>
            </div>
            <div class="collapse" id="customers-document-number-settings">
                <div class="card-body">
                    <p class="text-700 mb-3">{{ __('common.document_number_settings.description') }}</p>
                    <form class="js-business-document-number-settings-form"
                        action="{{ route($routePrefix.'.document-number-settings.update') }}"
                        method="POST"
                        novalidate>
                        @csrf
                        @method('PUT')
                        <div class="alert alert-danger alert-dismissible fade show d-none js-business-alert js-form-alert" role="alert">
                            <span class="js-business-alert-message js-form-alert-message"></span>
                            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="{{ __('auth.alerts.close') }}"></button>
                        </div>
                        <div class="row g-3 align-items-end">
                            <div class="col-md-6 col-lg-4">
                                <label class="form-label" for="customers-document-prefix">{{ __('common.document_number_settings.prefix') }}</label>
                                <x-forms.input class="form-control" id="customers-document-prefix" name="prefix" type="text" maxlength="20" value="{{ $documentNumberSettings['prefix'] ?? '' }}" />
                                <div class="invalid-feedback d-block" data-error-for="prefix"></div>
                            </div>
                            <div class="col-md-3 col-lg-2">
                                <label class="form-label" for="customers-document-padding">{{ __('common.document_number_settings.padding') }}</label>
                                <x-forms.input class="form-control" id="customers-document-padding" name="padding" type="number" min="0" max="10" step="1" value="{{ $documentNumberSettings['padding'] ?? 0 }}" required />
    ...
    <div class="card erp-datatable-card business-partner-datatable-card" data-business-partner-root data-resource="{{ $resource }}" data-bulk-delete-url="{{ route($routePrefix.'.bulk-delete') }}">
                ...
                <div class="col-6 col-sm-auto ms-auto text-end ps-0 d-flex justify-content-end align-items-center gap-2">
                    @can('customers.view_trashed')
                        <div class="d-flex align-items-center gap-2">
                            <label class="form-label mb-0 text-700 fs-10" for="customers_trash_filter">{{ __('business_partners.trash.filter_label') }}</label>
                            <x-forms.select class="form-select form-select-sm w-auto js-business-trash-filter" id="customers_trash_filter" aria-label="{{ __('business_partners.trash.filter_label') }}">
                    ...
                    @endcan
                    <x-buttons.add-record :href="route($routePrefix.'.create')" permission="customers.create" />
                </div>
            </div>
        </div>
            ...
            <div class="falcon-data-table">
                <div class="erp-datatable-wrapper">
                    <div class="erp-datatable-scroll">
                        <table id="customers-table" class="table table-sm table-hover mb-0 data-table erp-datatable align-middle js-business-partner-table"
                            ...
                            <thead class="bg-100 text-900">
                                <tr>
                                    ...
                                    @foreach($columns as $index => $column)
                                        @php
                                            $columnLabel = in_array($column, ['created_by', 'created_at', 'updated_by', 'updated_at'], true)
                                                ? __("common.fields.{$column}")
                                                : __("customers.columns.{$column}");
                                        @endphp
                                        <th class="text-900 sort pe-1 align-middle white-space-nowrap {{ $index === 0 ? 'all no-colvis dt-code' : 'dt-text dt-ellipsis' }}">{{ $columnLabel }}</th>
                                    @endforeach
...
@push('scripts')
    <script>
        ...
        window.businessPartnerCrudColumns = @json($columns);
