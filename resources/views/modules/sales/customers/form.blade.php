...
@php
    ...
    if (! $groupAccount && $linkedAccount) {
        $groupAccount = app(\Modules\Accounting\Services\BusinessPartnerAccountService::class)
            ->linkedAccountGroup(\Modules\Accounting\Services\BusinessPartnerAccountService::Customer, $linkedAccount);
    }
...
@section('content')
    <form class="js-business-partner-form js-crud-form" action="{{ $action }}" method="{{ $method }}" data-resource="customers" data-primary-focus="name" data-mode="{{ $mode }}" novalidate>
        @csrf
        @if($method !== 'POST')
            @method($method)
        @endif
        ...
        <div class="card mb-3 business-partner-form-card">
            <div class="card-header">
                <div class="row flex-between-center g-2">
                    ...
                    <div class="col-auto">@include('modules.finance.partials.form-actions', ['resource' => 'customers', 'routePrefix' => 'admin.sales.customers'])</div>
                </div>
            </div>
            <div class="card-body">
                <div class="alert d-none js-form-alert"><div class="js-form-alert-message"></div></div>
                <ul class="nav nav-tabs" role="tablist">
                    <li class="nav-item"><button class="nav-link active" data-bs-toggle="tab" data-bs-target="#customer-basic-tab" type="button" role="tab">{{ __('customers.tabs.basic_data') }}</button></li>
                    <li class="nav-item"><button class="nav-link" data-bs-toggle="tab" data-bs-target="#customer-credit-limits-tab" type="button" role="tab">{{ __('customers.tabs.credit_limits') }}</button></li>
                </ul>
                <div class="tab-content border-x border-bottom p-3">
                    <div class="tab-pane fade show active" id="customer-basic-tab" role="tabpanel">
                <div class="row g-3">
                    @if($canControlDocumentNumber)
                        <div class="col-md-2">
                            <label class="form-label" for="doc_number">{{ __('customers.attributes.doc_number') }}</label>
                            @if($isView)
                    ...
                    @elseif(! $isCreateLike)
                        <div class="col-md-2">
                            <x-forms.view-field for="doc_num" :label="__('customers.attributes.doc_num')" :value="$record?->doc_num" input-class="text-center" />
                        </div>
                    @endif

                    <div class="{{ $canControlDocumentNumber || ! $isCreateLike ? 'col-md-7' : 'col-md-9' }}">
                        <x-forms.label for="name" :label="__('customers.attributes.name')" required />
                        @if($isView)
                    ...
                    <div class="col-md-3">
                        <x-forms.label for="status" :label="__('customers.attributes.status')" required />
                        @if($isView)
                    ...
                    <div class="col-md-4">
                        <x-forms.label for="account_group_doc_num" :label="__('customers.attributes.account_group')" />
                        @if($isView)
                            <x-forms.view-field for="account_group_doc_num" :value="$groupOption['text'] ?? null" />
                        @else
                            <div class="business-partner-group-control">
                                <x-forms.select class="form-select js-select2-ajax js-business-partner-group-select" id="account_group_doc_num" name="account_group_doc_num" data-url="{{ route('admin.sales.select2.customer-groups') }}" data-placeholder="{{ __('customers.placeholders.customer_group') }}" data-allow-clear="true">
                                    @if($groupOption)
                                        <option value="{{ $groupOption['id'] }}" selected>{{ $groupOption['text'] }}</option>
                                    @endif
                                </x-forms.select>
                                @if($canCreateAccounts)
                                    <button class="btn btn-falcon-default btn-sm js-business-partner-inline-create" type="button" data-modal="#customer-inline-modal">
                                        <span class="fas fa-plus"></span><span class="ms-1">{{ __('customers.actions.add_group') }}</span>
                                    </button>
                                @endif
                            </div>
                            <div class="form-text">{{ __('customers.account_parent_help') }}</div>
                        @endif
                        <div class="invalid-feedback d-block" data-error-for="account_group_doc_num"></div>
                    </div>

                    <div class="col-md-4">
                        <label class="form-label" for="phone">{{ __('customers.attributes.phone') }}</label>
                        @if($isView)
                    ...
                    <div class="col-md-4">
                        <label class="form-label" for="mobile">{{ __('customers.attributes.mobile') }}</label>
                        @if($isView)
                    ...
                    <div class="col-md-4">
                        <label class="form-label" for="email">{{ __('customers.attributes.email') }}</label>
                        @if($isView)
                    ...
                    <div class="col-md-4">
                        <label class="form-label" for="tax_number">{{ __('customers.attributes.tax_number') }}</label>
                        @if($isView)
                    ...
                    <div class="col-md-4">
                        <label class="form-label" for="commercial_register">{{ __('customers.attributes.commercial_register') }}</label>
                        @if($isView)
                    ...
                    <div class="col-md-4">
                        <label class="form-label" for="national_id">{{ __('customers.attributes.national_id') }}</label>
                        @if($isView)
                    ...
                    <div class="col-md-4">
                        <label class="form-label" for="contact_person">{{ __('customers.attributes.contact_person') }}</label>
                        @if($isView)
                    ...
                    <div class="col-xl-3 col-md-6 business-partner-location-field">
                        <label class="form-label" for="country_doc_num">{{ __('customers.attributes.country') }}</label>
                        @if($isView)
                            <x-forms.view-field for="country_doc_num" :value="$locationOption('country')['text'] ?? null" />
                        @else
                            <div class="business-partner-group-control">
                                <x-forms.select class="form-select js-select2-ajax js-location-country" id="country_doc_num" name="country_doc_num" data-url="{{ route('admin.select2.countries') }}" data-placeholder="{{ __('customers.placeholders.country') }}" data-allow-clear="true">
                                    @if($locationOption('country'))<option value="{{ $locationOption('country')['id'] }}" selected>{{ $locationOption('country')['text'] }}</option>@endif
                                </x-forms.select>
                                <button class="btn btn-falcon-default btn-sm js-business-location-inline-create" type="button" data-type="countries" data-title="{{ __('customers.actions.add_country') }}" data-url="{{ route('admin.select2.inline.locations.store', 'countries') }}" data-target-select="#country_doc_num" data-modal="#customer-location-inline-modal"><span class="fas fa-plus"></span></button>
                            </div>
                        @endif
                    ...
                    <div class="col-xl-3 col-md-6 business-partner-location-field">
                        <label class="form-label" for="governorate_doc_num">{{ __('customers.attributes.governorate') }}</label>
                        @if($isView)
                            <x-forms.view-field for="governorate_doc_num" :value="$locationOption('governorate')['text'] ?? null" />
                        @else
                            <div class="business-partner-group-control">
                                <x-forms.select class="form-select js-select2-ajax js-location-governorate" id="governorate_doc_num" name="governorate_doc_num" data-url="{{ route('admin.select2.governorates') }}" data-placeholder="{{ __('customers.placeholders.governorate') }}" data-allow-clear="true">
                                ...
                                <button class="btn btn-falcon-default btn-sm js-business-location-inline-create" type="button" data-type="governorates" data-title="{{ __('customers.actions.add_governorate') }}" data-url="{{ route('admin.select2.inline.locations.store', 'governorates') }}" data-target-select="#governorate_doc_num" data-parent-select="#country_doc_num" data-parent-field="country_doc_num" data-modal="#customer-location-inline-modal"><span class="fas fa-plus"></span></button>
                            </div>
                        @endif
                    ...
                    <div class="col-xl-3 col-md-6 business-partner-location-field">
                        <label class="form-label" for="city_doc_num">{{ __('customers.attributes.city') }}</label>
                        @if($isView)
                            <x-forms.view-field for="city_doc_num" :value="$locationOption('cityLookup')['text'] ?? null" />
                        @else
                            <div class="business-partner-group-control">
                                <x-forms.select class="form-select js-select2-ajax js-location-city" id="city_doc_num" name="city_doc_num" data-url="{{ route('admin.select2.cities') }}" data-placeholder="{{ __('customers.placeholders.city') }}" data-allow-clear="true">
                                ...
                                <button class="btn btn-falcon-default btn-sm js-business-location-inline-create" type="button" data-type="cities" data-title="{{ __('customers.actions.add_city') }}" data-url="{{ route('admin.select2.inline.locations.store', 'cities') }}" data-target-select="#city_doc_num" data-parent-select="#governorate_doc_num" data-parent-field="governorate_doc_num" data-modal="#customer-location-inline-modal"><span class="fas fa-plus"></span></button>
                            </div>
                        @endif
                    ...
                    <div class="col-xl-3 col-md-6 business-partner-location-field">
                        <label class="form-label" for="area_doc_num">{{ __('customers.attributes.area') }}</label>
                        @if($isView)
                            <x-forms.view-field for="area_doc_num" :value="$locationOption('area')['text'] ?? null" />
                        @else
                            <div class="business-partner-group-control">
                                <x-forms.select class="form-select js-select2-ajax js-location-area" id="area_doc_num" name="area_doc_num" data-url="{{ route('admin.select2.areas') }}" data-placeholder="{{ __('customers.placeholders.area') }}" data-allow-clear="true">
                                    @if($locationOption('area'))<option value="{{ $locationOption('area')['id'] }}" selected>{{ $locationOption('area')['text'] }}</option>@endif
                                </x-forms.select>
                                <button class="btn btn-falcon-default btn-sm js-business-location-inline-create" type="button" data-type="areas" data-title="{{ __('customers.actions.add_area') }}" data-url="{{ route('admin.select2.inline.locations.store', 'areas') }}" data-target-select="#area_doc_num" data-parent-select="#city_doc_num" data-parent-field="city_doc_num" data-modal="#customer-location-inline-modal"><span class="fas fa-plus"></span></button>
                            </div>
                        @endif
                    ...
                    @if($linkedAccountDisplay)
                        <div class="col-12">
                            <x-forms.view-field for="linked_account" :label="__('customers.attributes.account')" :value="$linkedAccountDisplay" />
                        </div>
                    @endif

                    <div class="col-12">
                        <label class="form-label" for="address">{{ __('customers.attributes.address') }}</label>
                        @if($isView)
                    ...
                    <div class="col-12">
                        <label class="form-label" for="notes">{{ __('customers.attributes.notes') }}</label>
                        @if($isView)
                    ...
                    <div class="tab-pane fade business-partner-credit-tab" id="customer-credit-limits-tab" role="tabpanel">
                        @include('modules.business-partners.partials.credit-limits', ['rows' => $creditLimitRows, 'isView' => $isView, 'resource' => 'customers'])
                    </div>
                </div>
            ...
            <div class="card-footer">@include('modules.finance.partials.form-actions', ['resource' => 'customers', 'routePrefix' => 'admin.sales.customers'])</div>
        </div>
    </form>
    ...
    @unless($isView)
        <div class="modal fade" id="customer-inline-modal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog">
                <form class="modal-content js-business-partner-inline-form" action="{{ route('admin.sales.customers.account-groups.store') }}" method="POST" data-target-select="#account_group_doc_num" novalidate>
                    @csrf
                    <div class="modal-header">
                        <h5 class="modal-title">{{ __('customers.actions.add_group') }}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="{{ __('common.actions.close') }}"></button>
                    </div>
                    <div class="modal-body">
                        <div class="alert d-none js-form-alert"><div class="js-form-alert-message"></div></div>
                        <div class="mb-3">
                            <x-forms.label for="customer_inline_name" :label="__('customers.attributes.account_group_name')" required />
                            <x-forms.input class="form-control" id="customer_inline_name" name="name" type="text" required />
                            <div class="invalid-feedback" data-error-for="name"></div>
                        </div>
                        <div>
                            <label class="form-label" for="customer_inline_notes">{{ __('customers.attributes.notes') }}</label>
                            <x-forms.textarea class="form-control" id="customer_inline_notes" name="notes" rows="3"></x-forms.textarea>
                            <div class="invalid-feedback" data-error-for="notes"></div>
                        </div>
                    </div>
...
@push('scripts')
    ...
    <script src="{{ asset('assets/js/modules/Sales/customers.js') }}"></script>
@endpush
