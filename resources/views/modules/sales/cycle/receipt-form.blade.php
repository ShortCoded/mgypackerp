        ...
        <div class="card-body">
            <div class="row g-3 align-items-start">
                <div class="col-md-4">
                    ...
                    <x-forms.select class="form-select js-select2-ajax" id="customer_doc_num" name="customer_doc_num" required data-url="{{ route('admin.sales.select2.customers') }}" data-allow-clear="true">
                ...
                <div class="col-md-4">
                    ...
                    <x-forms.select class="form-select js-select2-ajax" id="received_by_employee_doc_num" name="received_by_employee_doc_num" required data-url="{{ route('admin.sales.select2.employees') }}" data-allow-clear="true" data-placeholder="{{ __('sales_ui.select_receiving_employee') }}">
                ...
                <div class="col-md-2">
                    ...
                    <x-forms.date-input class="form-control js-date-picker" id="receipt_date" name="receipt_date" value="{{ $formDate('receipt_date') }}" required />
                </div>
                <div class="col-md-2">
                    <label class="form-label" for="receipt_type">{{ __('Type') }} <span class="text-danger">*</span></label>
                    <x-forms.select class="form-select" id="receipt_type" name="receipt_type" required>
                ...
                <div class="col-md-3">
                    ...
                    <x-forms.select class="form-select js-select2-ajax" id="currency_doc_num" name="currency_doc_num" data-url="{{ route('admin.select2.currencies') }}" data-placeholder="{{ __('Currency') }}" required>
                ...
                <div class="col-md-3">
                    ...
                    <x-forms.input class="form-control text-end" id="amount" name="amount" value="{{ old('amount', $numbers->formatForInput($suggestedAmount)) }}" inputmode="decimal" required />
                </div>
                <div class="col-md-3">
                    <label class="form-label" for="payment_method">{{ __('Payment method') }} <span class="text-danger">*</span></label>
                    <x-forms.select class="form-select" id="payment_method" name="payment_method" data-sales-payment-method required>
                ...
                <div class="col-md-3" data-payment-source="cash">
                    ...
                    <x-forms.select class="form-select js-select2-ajax" data-url="{{ route('admin.sales.select2.cashboxes') }}" data-required-for="cash" id="cashbox_doc_num" name="cashbox_doc_num">
                ...
                <div class="col-md-3 d-none" data-payment-source="bank">
                    ...
                    <x-forms.select class="form-select js-select2-ajax" data-url="{{ route('admin.sales.select2.bank-accounts') }}" data-required-for="bank,transfer,cheque" id="bank_account_doc_num" name="bank_account_doc_num" disabled>
                ...
                <div class="col-md-3 d-none" data-payment-source="reference">
                    ...
                    <x-forms.input class="form-control" id="reference_no" name="reference_no" value="{{ old('reference_no') }}" data-required-for="cheque" dir="ltr" disabled />
                </div>
                <div class="col-md-3 d-none" data-payment-source="cheque">
                    ...
                    <x-forms.date-input class="form-control js-date-picker" id="cheque_due_date" name="cheque_due_date" value="{{ $formDate('cheque_due_date') }}" data-required-for="cheque" disabled />
                </div>
                <div class="col-md-3 d-none" data-payment-source="cheque">
                    <label class="form-label" for="external_bank_name">{{ __('Drawer bank') }} <span class="text-danger">*</span></label>
                    <x-forms.input class="form-control" id="external_bank_name" name="external_bank_name" value="{{ old('external_bank_name') }}" data-required-for="cheque" disabled />
                </div>
                <div class="col-12"><label class="form-label" for="notes">{{ __('Notes') }}</label><x-forms.textarea class="form-control" id="notes" name="notes" rows="2">{{ old('notes') }}</x-forms.textarea></div>
                @if($selectedOrder)<x-forms.input type="hidden" name="sales_order_doc_num" value="{{ $selectedOrder->doc_num }}" />@endif
            </div>
        </div>
    </div>
        ...
        <div class="table-responsive"><table class="table table-sm table-bordered align-middle mb-0" style="min-width:850px"><thead><tr><th></th><th>{{ __('Invoice') }}</th><th>{{ __('Customer') }}</th><th>{{ __('Due date') }}</th><th class="text-end">{{ __('Outstanding') }}</th><th>{{ __('Allocate') }}</th></tr></thead><tbody>
            @forelse($schedules as $index => $schedule)
                <tr data-receipt-allocation-row data-customer="{{ $schedule->invoice->customer->doc_num }}">
                    <td><x-forms.input class="form-check-input" type="checkbox" data-receipt-allocation-toggle /></td>
                    ...
                    <td><x-forms.input type="hidden" name="allocations[{{ $index }}][invoice_schedule_public_id]" value="{{ $schedule->public_id }}" disabled /><x-forms.input class="form-control form-control-sm text-end" name="allocations[{{ $index }}][amount]" inputmode="decimal" max="{{ $schedule->outstanding_amount }}" disabled /></td>
                </tr>
