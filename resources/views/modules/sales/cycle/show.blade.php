    ...
    @if($kind === 'production_request')
        <div class="card mb-3"><div class="card-header d-flex justify-content-between"><h6 class="mb-0">{{ __('BOM snapshots and production runs') }}</h6><div class="d-flex gap-2">@can('production.orders.print')<a class="btn btn-falcon-default btn-sm" href="{{ route('admin.production.work-orders.requirement.print', $record) }}">{{ __('Print requirement') }}</a>@endcan @if(in_array($record->status, ['draft', 'planned']))<form method="POST" action="{{ route('admin.production.work-orders.release', $record) }}">@csrf<button class="btn btn-primary btn-sm">{{ __('Release and snapshot BOM') }}</button></form>@endif</div></div><div class="card-body">
            @foreach($record->lines as $line)
                <div class="mb-3"><strong>{{ $line->product?->doc_num }} — {{ $line->product?->name }}</strong><div class="text-500">{{ __('Target base quantity') }}: {{ $numbers->format($line->base_quantity) }} · {{ __('Received') }}: {{ $numbers->format($line->received_base_quantity) }}</div>
                    @if(is_array($line->bom_snapshot))<ul class="mb-0">@foreach($line->bom_snapshot['components'] ?? [] as $component)<li>{{ $component['product_doc_num'] ?? '' }} — {{ $component['product_name'] ?? '' }}: {{ $numbers->format($component['base_quantity_per_output'] ?? null) }} / {{ __('base output unit') }}</li>@endforeach</ul>@endif
                </div>
            @endforeach
            <div class="row g-2">@forelse($record->runs as $run)<div class="col-md-4"><a class="d-block border rounded p-2" href="{{ route('admin.production.runs.show', $run) }}"><strong>{{ $run->run_number }}</strong><br><span>{{ __(str($run->status)->replace('_', ' ')->title()->toString()) }} · {{ $numbers->format($run->planned_base_quantity) }} / {{ $numbers->format($run->good_base_quantity) }} / {{ $numbers->format($run->received_base_quantity) }}</span></a></div>@empty<div class="text-500">{{ __('No production runs planned.') }}</div>@endforelse</div>
        </div></div>
    @endif
        ...
        <div class="card mb-3"><div class="card-header"><h6 class="mb-0">{{ __('Customer Credit Allocation and Refund') }}</h6></div><div class="card-body">
            @if($kind === 'credit_note')
                ...
                @if((float) $record->credit_available_amount > 0)
                    <div class="row g-3">
                        @can('customer_credits.allocate')<div class="col-xl-6"><form class="js-sales-cycle-action border rounded p-3" action="{{ route('admin.sales.sales-invoices.credit-allocations.store', $record) }}" method="POST">@csrf<h6>{{ __('Apply to Future Invoice') }}</h6><div class="row g-2"><div class="col-md-6"><label class="form-label">{{ __('Target Invoice') }}</label><x-forms.select name="target_invoice_doc_num" variant="ajax" :url="route('admin.sales.select2.credit-target-invoices')" :data-extra-params="json_encode(['customer_doc_num' => $record->customer?->doc_num])" :placeholder="__('Target Invoice')" :allow-clear="false" required></x-forms.select></div><div class="col-md-3"><x-forms.label :label="__('Amount')" required /><x-forms.numeric-input name="amount" :scale="4" min="0.0001" step="0.0001" required /></div><div class="col-md-3"><label class="form-label">{{ __('Date') }}</label><x-forms.date-input name="allocation_date" :value="today()" required /></div><div class="col-12"><button class="btn btn-primary btn-sm" type="submit">{{ __('Apply Credit') }}</button></div></div></form></div>@endcan
                        @can('customer_credits.refund')<div class="col-xl-6"><form class="js-sales-cycle-action border rounded p-3" action="{{ route('admin.sales.sales-invoices.credit-refunds.store', $record) }}" method="POST">@csrf<h6>{{ __('Refund Remaining Credit') }}</h6><div class="row g-2"><div class="col-md-4"><x-forms.label :label="__('Amount')" required /><x-forms.numeric-input name="amount" :scale="4" min="0.0001" step="0.0001" required /></div><div class="col-md-4"><label class="form-label">{{ __('Date') }}</label><x-forms.date-input name="refund_date" :value="today()" required /></div><div class="col-md-4"><label class="form-label">{{ __('Method') }}</label><x-forms.select name="payment_method" variant="local" :allow-clear="false" required><option value="cash">{{ __('Cash') }}</option><option value="bank">{{ __('Bank') }}</option></x-forms.select></div><div class="col-md-6"><label class="form-label">{{ __('Cashbox (for cash)') }}</label><x-forms.select name="cashbox_doc_num" variant="ajax" :url="route('admin.sales.select2.cashboxes')"><option value="">—</option></x-forms.select></div><div class="col-md-6"><label class="form-label">{{ __('Bank account (for bank)') }}</label><x-forms.select name="bank_account_doc_num" variant="ajax" :url="route('admin.sales.select2.bank-accounts')"><option value="">—</option></x-forms.select></div><div class="col-12"><button class="btn btn-warning btn-sm" type="submit">{{ __('Post Refund') }}</button></div></div></form></div>@endcan
                    </div>
                @endif
