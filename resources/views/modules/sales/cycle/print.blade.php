    ...
    <div class="card erp-document-print sales-cycle-print" dir="{{ app()->isLocale('ar') ? 'rtl' : 'ltr' }}">
        <div class="card-body">
            ...
            <div class="d-flex justify-content-between align-items-start mb-4">
                ...
                <div class="text-end"><span class="badge bg-secondary">{{ str($record->status)->replace('_', ' ')->title() }}</span><div class="mt-2">{{ $dates->formatDate($documentDate, '') }}</div></div>
            </div>
            <div class="row g-3 mb-4">
                ...
                @if($record->reason_code ?? null)<div class="col-12"><strong>{{ __('Return reason') }}</strong><div>{{ str($record->reason_code)->replace('_', ' ')->title() }} — {{ $record->reason_details }}</div></div>@endif
                @if($record->salesReturn ?? null)<div class="col-12"><strong>{{ __('Credit reason / Return') }}</strong><div>{{ $record->salesReturn->doc_num }} — {{ str($record->salesReturn->reason_code)->replace('_', ' ')->title() }}</div></div>@endif
                @if($kind === 'customer_receipt')<div class="col-6"><strong>{{ __('Payment method') }}</strong><div>{{ str($record->payment_method)->replace('_', ' ')->title() }}</div></div><div class="col-6"><strong>{{ __('Cash / Bank / Cheque reference') }}</strong><div>{{ $record->cashVoucher?->doc_num ?? $record->cheque?->doc_num ?? $record->bankAccount?->doc_num ?? $record->reference_no ?? '—' }}</div></div><div class="col-6"><strong>{{ __('Receipt amount') }}</strong><div dir="ltr">{{ $numbers->format($record->amount) }} {{ $record->currency?->code }}</div></div>@endif
                @if(in_array($kind, ['invoice', 'credit_note'], true))<div class="col-6"><strong>{{ __('Electronic invoice status') }}</strong><div>{{ str($record->electronic_invoice_status)->replace('_', ' ')->title() }}</div></div>@endif
