...
@section('content')
<form class="js-sales-cycle-form" action="{{ route('admin.sales.sales-invoices.update', $record) }}" method="POST" novalidate>
    @csrf @method('PUT')
    ...
    <div class="d-flex justify-content-end gap-2"><a class="btn btn-falcon-default" href="{{ route('admin.sales.sales-invoices.show', $record) }}">{{ __('Cancel') }}</a><button class="btn btn-primary" type="submit">{{ __('Save Correction') }}</button></div>
</form>
@endsection

@push('scripts')<script src="{{ asset('assets/js/modules/Sales/sales-cycle.js') }}"></script>@endpush
