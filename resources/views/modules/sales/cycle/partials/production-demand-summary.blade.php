...
@endphp
<div class="card my-3"><div class="card-header"><h5>{{ __('Production demand by product') }}</h5></div><div class="table-responsive"><table class="table table-sm report-table"><thead><tr><th>{{ __('Item') }}</th><th>{{ __('Warehouse') }}</th><th>{{ __('Base unit') }}</th><th>{{ __('Shortage') }}</th><th>{{ __('Remaining production') }}</th><th>{{ __('Unplanned quantity') }}</th><th>{{ __('Source orders') }}</th></tr></thead><tbody>
