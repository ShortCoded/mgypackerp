    ...
    @if($record->posting_status === 'posted')
        ...
        @if($record->canReopenSafely()) @can('customer_invoices.reopen')
            <details class="card mb-3" data-invoice-action-panel>
                ...
                <form data-sales-ui class="js-sales-cycle-action card-body py-3 border-top" action="{{ route('admin.sales.sales-invoices.reopen', $record) }}" method="POST">@csrf<x-forms.line-item-cards :line-label="__('sales_ui.line')" /><x-forms.label for="invoice_correction_reason" :label="__('Correction reason')" required /><textarea class="form-control form-control-sm mb-2" id="invoice_correction_reason" name="reason" required></textarea><button class="btn btn-warning btn-sm" type="submit">{{ __('Reverse and Reopen') }}</button><small class="d-block text-600 mt-2">{{ __('Allowed only while the invoice has no receipts or credit notes. The original journal is reversed before amendment.') }}</small></form>
            </details>
        @endcan @endif
        @can('sales_returns.create')
            <details class="card mb-3" data-invoice-action-panel>
                ...
                <form id="sales-invoice-return" data-sales-ui class="js-sales-cycle-action card-body py-3 border-top" action="{{ route('admin.sales.sales-returns.store', $record) }}" method="POST">
                    ...
                    <div class="row g-2 mb-3">
                        <div class="col-md-4"><x-forms.label for="invoice_return_reason" :label="__('Return reason')" required /><select class="form-select form-select-sm" id="invoice_return_reason" name="reason_code" required><option value="customer_rejection">{{ __('Customer rejection') }}</option><option value="manufacturing_defect">{{ __('Manufacturing defect') }}</option><option value="damaged_goods">{{ __('Damaged goods') }}</option><option value="wrong_specification">{{ __('Wrong specification') }}</option><option value="other">{{ __('Other') }}</option></select></div>
                        <div class="col-md-8"><x-forms.label for="invoice_return_details" :label="__('Details')" /><input class="form-control form-control-sm" id="invoice_return_details" name="reason_details"></div>
                                    ...
                                    <div class="row g-2 align-items-center">
                                        <div class="col-md-8">
                                            <div class="form-check mb-1">
                                                <input class="form-check-input" id="{{ $returnToggleId }}" type="checkbox" data-sales-return-toggle>
                                            ...
                                            <input type="hidden" name="lines[{{ $index }}][invoice_line_public_id]" value="{{ $line->public_id }}" disabled>
                                        </div>
                                        <div class="col-md-4"><x-forms.label :for="$returnQuantityId" :label="__('sales_ui.return_quantity')" required /><input class="form-control form-control-sm text-end" id="{{ $returnQuantityId }}" name="lines[{{ $index }}][quantity]" value="{{ $numbers->formatForInput($returnable) }}" inputmode="decimal" min="0.00000001" max="{{ $returnable }}" disabled required></div>
                                    </div>
                                </div>
                            @endif
                        @endforeach
