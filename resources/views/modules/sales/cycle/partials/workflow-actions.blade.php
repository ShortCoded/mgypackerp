    ...
    <div class="card mb-3">
        ...
        <div class="card-body">
            ...
            <div class="row g-3">
                ...
                @if($canReopenOrder) @can('sales_orders.reopen')<div class="col-lg-4"><form data-sales-ui class="js-sales-cycle-action border rounded p-3" action="{{ route('admin.sales.sales-orders.reopen', $record) }}" method="POST">@csrf<x-forms.line-item-cards :line-label="__('sales_ui.line')" /><label class="form-label">{{ __('Reopen reason') }}</label><x-forms.textarea class="form-control form-control-sm mb-2" name="reason" required></x-forms.textarea><button class="btn btn-falcon-warning btn-sm" type="submit">{{ __('Reopen for Amendment') }}</button></form></div>@endcan @endif
...
@elseif(in_array($kind, ['invoice', 'credit_note'], true))
    ...
    @if($record->posting_status === 'posted')
            ...
            <details class="card mb-3" data-invoice-action-panel>
                ...
                <form id="sales-invoice-delivery" data-sales-ui class="js-sales-cycle-action card-body py-3 border-top" action="{{ route('admin.sales.sales-invoices.deliveries.store', $record) }}" method="POST">
                    ...
                    <div class="row g-2 mb-3">
                        <div class="col-md-4"><x-forms.label for="delivery_branch_store_uuid" :label="__('Delivery warehouse')" required /><x-forms.select class="form-select js-select2-ajax" id="delivery_branch_store_uuid" name="branch_store_uuid" data-url="{{ route('admin.sales.select2.stores') }}" data-placeholder="{{ __('Select store') }}" required></x-forms.select></div>
                        <div class="col-md-2"><x-forms.label for="delivery_document_date" :label="__('Date')" required /><x-forms.date-input class="form-control js-date-picker" id="delivery_document_date" name="document_date" value="{{ $dates->formatDate($today, '') }}" required /></div>
        ...
        @if($record->canReopenSafely()) @can('customer_invoices.reopen')
            <details class="card mb-3" data-invoice-action-panel>
                <summary class="card-header py-2 fw-semibold">{{ __('Reverse and Reopen') }}</summary>
                <form data-sales-ui class="js-sales-cycle-action card-body py-3 border-top" action="{{ route('admin.sales.sales-invoices.reopen', $record) }}" method="POST">@csrf<x-forms.line-item-cards :line-label="__('sales_ui.line')" /><x-forms.label for="invoice_correction_reason" :label="__('Correction reason')" required /><x-forms.textarea class="form-control form-control-sm mb-2" id="invoice_correction_reason" name="reason" required></x-forms.textarea><button class="btn btn-warning btn-sm" type="submit">{{ __('Reverse and Reopen') }}</button><small class="d-block text-600 mt-2">{{ __('Allowed only while the invoice has no receipts or credit notes. The original journal is reversed before amendment.') }}</small></form>
            </details>
        @endcan @endif
            ...
            <details class="card mb-3" data-invoice-action-panel>
                ...
                <form id="sales-invoice-return" data-sales-ui class="js-sales-cycle-action card-body py-3 border-top" action="{{ route('admin.sales.sales-returns.store', $record) }}" method="POST">
                    ...
                    <div class="row g-2 mb-3">
                        ...
                        @if($record->lines->contains(fn ($line) => ! $line->is_service))<div class="col-md-6"><x-forms.label for="return_branch_store_uuid" :label="__('Return warehouse')" required /><x-forms.select class="form-select js-select2-ajax" id="return_branch_store_uuid" name="branch_store_uuid" data-url="{{ route('admin.sales.select2.stores') }}" data-placeholder="{{ __('Select store') }}" required></x-forms.select></div>@endif
                    </div>
                    <fieldset>
