...
<tr data-sales-line>
    ...
    <td><select class="form-select form-select-sm js-sales-product" name="lines[{{ $index }}][product_doc_num]" required><option value="">{{ __('Select product') }}</option>@foreach($products as $product)<option value="{{ $product->doc_num }}" @selected($selectedProduct === $product->doc_num)>{{ $product->doc_num }} / {{ $product->name }}</option>@endforeach</select><input class="form-control form-control-sm mt-1" name="lines[{{ $index }}][description]" value="{{ $line['description'] ?? '' }}" placeholder="{{ __('Line description') }}"><div class="invalid-feedback d-block" data-error-for="lines.{{ $index }}.product_doc_num"></div></td>
    <td><select class="form-select form-select-sm js-sales-unit" name="lines[{{ $index }}][unit_doc_num]" required>@foreach(($productUnits[$selectedProduct] ?? collect()) as $unit)<option value="{{ $unit['id'] }}" @selected($selectedUnit === $unit['id'])>{{ $unit['text'] }}</option>@endforeach</select></td>
    <td><input class="form-control form-control-sm text-end js-sales-quantity" name="lines[{{ $index }}][quantity]" value="{{ $line['quantity'] ?? '' }}" inputmode="decimal" required></td>
    <td><input class="form-control form-control-sm text-end js-sales-price" name="lines[{{ $index }}][unit_price]" value="{{ $line['unit_price'] ?? '' }}" inputmode="decimal" required></td>
    ...
    <td class="text-end fw-semibold" dir="ltr" data-sales-line-total>0.00</td>
    ...
    <td><button class="btn btn-link text-danger p-1" type="button" data-sales-remove-row title="{{ __('Remove') }}">&times;</button></td>
</tr>
