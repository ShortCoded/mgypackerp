...
<div data-sales-ui>
    <x-admin.report.page :title="$reportTitle" :description="$reportDescription">
        ...
        <x-admin.report.filter-panel
            ...
            @if(filled($filters['operational_focus'] ?? null))<x-forms.input type="hidden" name="operational_focus" value="{{ $filters['operational_focus'] }}" />@endif
        ...
        @if($reportType === 'period')
            <x-admin.report.table-card :title="__('Sales by Period')" table-id="sales-by-period" class="mb-3"><thead><tr><th>{{ __('Date') }}</th><th class="text-end">{{ __('Invoices') }}</th><th class="text-end">{{ __('Value') }}</th></tr></thead><tbody>@forelse($salesByPeriod as $row)<tr><td>{{ $dateValue($row->invoice_date) }}</td><td class="text-end">{{ $row->invoice_count }}</td><td class="text-end">{{ $numbers->format($row->sales_value) }}</td></tr>@empty{!! $emptyRow(3) !!}@endforelse</tbody><tfoot><tr class="fw-bold"><td class="text-end">{{ __('Totals') }}</td><td class="text-end" data-period-total="invoice_count">{{ $periodSummary['invoice_count'] ?? 0 }}</td><td class="text-end" data-period-total="sales_value">{{ $numbers->format($periodSummary['sales_value'] ?? 0) }}</td></tr></tfoot></x-admin.report.table-card>
        @endif

        @if($reportType === 'products')
            <div class="row g-3 mb-3"><div class="col-xl-6"><x-admin.report.table-card :title="__('Sales by Item')" table-id="sales-by-product" class="h-100"><thead><tr><th>{{ __('Item') }}</th><th class="text-end">{{ __('Quantity') }}</th><th class="text-end">{{ __('Value') }}</th></tr></thead><tbody>@forelse($salesByItem as $row)<tr><td>{{ $row->doc_num }} / {{ $row->name }}</td><td class="text-end">{{ $numbers->format($row->sold_quantity) }}</td><td class="text-end">{{ $numbers->format($row->sales_value) }}</td></tr>@empty{!! $emptyRow(3) !!}@endforelse</tbody><tfoot><tr class="fw-bold"><td class="text-end">{{ __('Totals') }} ({{ $productSummary['product_count'] ?? 0 }})</td><td class="text-end" data-product-total="sold_quantity">{{ $numbers->format($productSummary['sold_quantity'] ?? 0) }}</td><td class="text-end" data-product-total="sales_value">{{ $numbers->format($productSummary['sales_value'] ?? 0) }}</td></tr></tfoot></x-admin.report.table-card></div><div class="col-xl-6"><x-admin.report.table-card :title="__('Customer / Item Sales Analysis')" table-id="customer-product-sales" class="h-100"><thead><tr><th>{{ __('Customer') }}</th><th>{{ __('Item') }}</th><th class="text-end">{{ __('Quantity') }}</th><th class="text-end">{{ __('Value') }}</th></tr></thead><tbody>@forelse($salesByCustomerItem as $row)<tr><td>{{ $row->customer_doc_num }} / {{ $row->customer_name }}</td><td>{{ $row->product_doc_num }} / {{ $row->product_name }}</td><td class="text-end">{{ $numbers->format($row->sold_quantity) }}</td><td class="text-end">{{ $numbers->format($row->sales_value) }}</td></tr>@empty{!! $emptyRow(4) !!}@endforelse</tbody><tfoot><tr class="fw-bold"><td colspan="2" class="text-end">{{ __('Totals') }} ({{ $customerProductSummary['line_count'] ?? 0 }})</td><td class="text-end" data-customer-product-total="sold_quantity">{{ $numbers->format($customerProductSummary['sold_quantity'] ?? 0) }}</td><td class="text-end" data-customer-product-total="sales_value">{{ $numbers->format($customerProductSummary['sales_value'] ?? 0) }}</td></tr></tfoot></x-admin.report.table-card></div></div>
        @endif

        @if($reportType === 'operational')
            <div class="card mb-3"><div class="card-body d-flex align-items-center justify-content-between"><span class="fw-semibold">{{ __('sales_ui.reports.operational.actionable_total') }}</span><span class="fs-4 fw-bold" data-report-count="pending_sales_actions">{{ $salesActionCount }}</span></div></div>
            <x-admin.report.table-card :title="__('sales_ui.reports.operational.sales_requests')" table-id="pending-sales-requests" class="mb-3"><thead><tr><th>{{ __('sales_ui.reports.columns.document') }}</th><th>{{ __('sales_ui.reports.columns.date') }}</th><th>{{ __('sales_ui.reports.columns.customer') }}</th><th>{{ __('sales_ui.reports.columns.status') }}</th><th class="text-end">{{ __('sales_ui.reports.operational.remaining_quantity') }}</th></tr></thead><tbody>@forelse($salesRequests as $salesRequest)<tr><td><a href="{{ route('admin.sales.sales-requests.show', $salesRequest) }}">{{ $salesRequest->doc_num }}</a></td><td>{{ $dateValue($salesRequest->request_date) }}</td><td>{{ $salesRequest->customer?->name ?: '—' }}</td><td>{{ __('sales_ui.reports.operational.statuses.'.$salesRequest->status) }}</td><td class="text-end">{{ $numbers->format($salesRequest->remaining_quantity) }}</td></tr>@empty{!! $emptyRow(5) !!}@endforelse</tbody></x-admin.report.table-card>
        @endif

        @if($reportType === 'invoices' || ($reportType === 'operational' && empty($filters['operational_focus'])))
            <x-admin.report.table-card :title="__('Sales Ledger')" table-id="sales-invoices-ledger" class="mb-3"><thead><tr><th>{{ __('Invoice') }}</th><th>{{ __('Customer') }}</th><th>{{ __('Date') }}</th><th>{{ __('Sales Order') }}</th><th>{{ __('Issue Orders') }}</th><th class="text-end">{{ __('Net Sales') }}</th><th class="text-end">{{ __('Collected') }}</th><th class="text-end">{{ __('Outstanding') }}</th></tr></thead><tbody>@forelse($salesLedger as $invoice)<tr><td><a href="{{ route('admin.sales.sales-invoices.show', $invoice) }}">{{ $invoice->doc_num }}</a></td><td>{{ $invoice->customer?->name }}</td><td>{{ $dateValue($invoice->invoice_date) }}</td><td>{{ $invoice->order?->doc_num ?: '—' }}</td><td>{{ $invoice->deliveries->pluck('doc_num')->join(' / ') ?: '—' }}</td><td class="text-end">{{ $numbers->format(bcsub((string) $invoice->total_amount, (string) ($invoice->returns_amount ?? 0), 4)) }}</td><td class="text-end">{{ $numbers->format($invoice->paid_amount) }}</td><td class="text-end">{{ $numbers->format($invoice->remaining_amount) }}</td></tr>@empty{!! $emptyRow(8) !!}@endforelse</tbody><tfoot><tr class="fw-bold"><td colspan="5" class="text-end">{{ __('Totals') }} ({{ $ledgerSummary['invoice_count'] ?? 0 }})</td><td class="text-end" data-ledger-total="net_sales">{{ $numbers->format($ledgerSummary['net_sales'] ?? 0) }}</td><td class="text-end" data-ledger-total="collected">{{ $numbers->format($ledgerSummary['collected'] ?? 0) }}</td><td class="text-end" data-ledger-total="outstanding">{{ $numbers->format($ledgerSummary['outstanding'] ?? 0) }}</td></tr></tfoot></x-admin.report.table-card>
        ...
        @if($reportType === 'returns')
            <div class="row g-3 mb-3"><div class="col-xl-6"><x-admin.report.table-card :title="__('Returns by Reason and Quality Disposition')" table-id="returns-summary" class="h-100"><thead><tr><th>{{ __('Reason') }}</th><th class="text-end">{{ __('Returns') }}</th><th class="text-end">{{ __('Quantity') }}</th><th class="text-end">{{ __('Saleable') }}</th><th class="text-end">{{ __('Rejected / Rework / Scrap') }}</th></tr></thead><tbody>@forelse($returns as $row)<tr><td>{{ __(str($row->reason_code)->replace('_', ' ')->title()->toString()) }}</td><td class="text-end">{{ $row->return_count }}</td><td class="text-end">{{ $numbers->format($row->returned_quantity) }}</td><td class="text-end">{{ $numbers->format($row->saleable_quantity) }}</td><td class="text-end">{{ $numbers->format($row->rejected_quantity) }}</td></tr>@empty{!! $emptyRow(5) !!}@endforelse</tbody><tfoot><tr class="fw-bold"><td class="text-end">{{ __('Totals') }} ({{ $returnsSummary['return_count'] ?? 0 }})</td><td></td><td class="text-end" data-returns-total="returned_quantity">{{ $numbers->format($returnsSummary['returned_quantity'] ?? 0) }}</td><td class="text-end" data-returns-total="saleable_quantity">{{ $numbers->format($returnsSummary['saleable_quantity'] ?? 0) }}</td><td class="text-end" data-returns-total="rejected_quantity">{{ $numbers->format($returnsSummary['rejected_quantity'] ?? 0) }}</td></tr></tfoot></x-admin.report.table-card></div><div class="col-xl-6"><x-admin.report.table-card :title="__('Customer / Item Return Analysis')" table-id="return-analysis" class="h-100"><thead><tr><th>{{ __('Customer') }}</th><th>{{ __('Item') }}</th><th>{{ __('Reason') }}</th><th>{{ __('Disposition') }}</th><th class="text-end">{{ __('Quantity') }}</th></tr></thead><tbody>@forelse($returnAnalysis as $row)<tr><td>{{ $row->customer_name }}</td><td>{{ $row->product_name }}</td><td>{{ __(str($row->reason_code)->replace('_', ' ')->title()->toString()) }}</td><td>{{ $row->quality_disposition ? $qualityDispositionLabel($row->quality_disposition) : '—' }}</td><td class="text-end">{{ $numbers->format($row->returned_quantity) }}</td></tr>@empty{!! $emptyRow(5) !!}@endforelse</tbody><tfoot><tr class="fw-bold"><td colspan="4" class="text-end">{{ __('Totals') }} ({{ $returnAnalysisSummary['line_count'] ?? 0 }})</td><td class="text-end" data-return-analysis-total="returned_quantity">{{ $numbers->format($returnAnalysisSummary['returned_quantity'] ?? 0) }}</td></tr></tfoot></x-admin.report.table-card></div></div>
        @endif

        @if($reportType === 'cost_of_sales')
            <div class="mb-3">
                <x-admin.report.table-card :title="__('sales_ui.reports.types.cost_of_sales')" table-id="cost-of-sales">
                    <thead><tr>
                        <th>{{ __('sales_ui.reports.export.headings.movement_kind') }}</th>
                        <th>{{ __('sales_ui.reports.export.headings.document') }}</th>
                        <th>{{ __('sales_ui.reports.export.headings.return_document') }}</th>
                        <th>{{ __('sales_ui.reports.export.headings.order') }}</th>
                        <th>{{ __('sales_ui.reports.export.headings.invoice') }}</th>
                        <th>{{ __('sales_ui.reports.export.headings.customer') }}</th>
                        <th>{{ __('sales_ui.reports.export.headings.product') }}</th>
                        <th class="text-end">{{ __('sales_ui.reports.export.headings.quantity') }}</th>
                        <th class="text-end">{{ __('sales_ui.reports.export.headings.unit_cost') }}</th>
                        <th class="text-end">{{ __('sales_ui.reports.export.headings.total_cost') }}</th>
                        <th>{{ __('sales_ui.reports.export.headings.posting_date') }}</th>
                        <th>{{ __('sales_ui.reports.export.headings.journal') }}</th>
                        <th>{{ __('sales_ui.reports.export.headings.reconciliation') }}</th>
                    </tr></thead>
                    <tbody>
                    @forelse($costOfSalesRows as $row)
                        <tr>
                            <td>{{ __(str($row['movement_kind'])->replace('_', ' ')->title()->toString()) }}</td>
                            <td>{{ $row['document'] }}</td>
                            <td>{{ $row['return_document'] ?? '—' }}</td>
                            <td>{{ $row['order'] ?? '—' }}</td>
                            <td>{{ $row['invoice'] ?? '—' }}</td>
                            <td>{{ $row['customer'] ?? '—' }}</td>
                            <td>{{ $row['product'] ?? '—' }}</td>
                            <td class="text-end">{{ $numbers->format($row['quantity']) }}</td>
                            <td class="text-end">{{ $row['unit_cost'] !== null ? $numbers->format($row['unit_cost']) : '—' }}</td>
                            <td class="text-end">{{ $row['signed_total_cost'] !== null ? $numbers->format($row['signed_total_cost']) : '—' }}</td>
                            <td>{{ $dateValue($row['posting_date']) }}</td>
                            <td>{{ $row['journal_entry'] ?? '—' }}</td>
                            <td>{{ __(str($row['reconciliation_status'])->replace('_', ' ')->title()->toString()) }}</td>
                        </tr>
                    @empty
                        {!! $emptyRow(13) !!}
                    @endforelse
                    ...
                    <tfoot><tr class="fw-bold">
                        <td colspan="7">{{ __('Totals') }} ({{ $costOfSalesSummary['delivery_count'] ?? 0 }} {{ __('sales_ui.reports.export.metrics.deliveries') }} / {{ $costOfSalesSummary['return_count'] ?? 0 }} {{ __('sales_ui.reports.export.metrics.returns') }})</td>
                        <td></td>
                        <td></td>
                        <td class="text-end" data-cost-total="net_cost">{{ $numbers->format($costOfSalesSummary['net_cost'] ?? '0.0000') }}</td>
                        <td colspan="3">{{ $costOfSalesSummary['unreconciled_count'] ?? 0 }} {{ __('sales_ui.reports.export.metrics.unreconciled') }}</td>
                    </tr></tfoot>
