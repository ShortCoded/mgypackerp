...
<div class="sales-cycle-print" dir="{{ app()->getLocale() === 'ar' ? 'rtl' : 'ltr' }}">
    ...
    <div class="card mb-3">
        <div class="card-header"><h5 class="mb-0">{{ __('Sales Cycle Operational Report') }}</h5></div>
        <div class="card-body report-filter-fields">
            <form class="row g-3">
                <div class="col-md-3"><label class="form-label">{{ __('From') }}</label><input class="form-control datetimepicker" name="from" value="{{ request('from') }}"></div>
                <div class="col-md-3"><label class="form-label">{{ __('To') }}</label><input class="form-control datetimepicker" name="to" value="{{ request('to') }}"></div>
                ...
                <div class="col-md-3"><label class="form-label">{{ __('Quotation Status') }}</label><select class="form-select" name="quotation_status"><option value="">{{ __('All') }}</option>@foreach(\Modules\Sales\Models\Quotation::Statuses as $status)<option value="{{ $status }}" @selected($filters['quotation_status'] === $status)>{{ str($status)->replace('_', ' ')->title() }}</option>@endforeach</select></div>
                <div class="col-md-3"><label class="form-label">{{ __('Order Status') }}</label><select class="form-select" name="order_status"><option value="">{{ __('All') }}</option>@foreach($orderStatuses as $status)<option value="{{ $status }}" @selected($filters['order_status'] === $status)>{{ str($status)->replace('_', ' ')->title() }}</option>@endforeach</select></div>
    ...
    <div class="card mt-3"><div class="card-header"><h6 class="mb-0">{{ __('Returns by Reason and Quality Disposition') }}</h6></div><div class="table-responsive"><table class="table table-sm mb-0"><thead><tr><th>{{ __('Reason') }}</th><th class="text-end">{{ __('Returns') }}</th><th class="text-end">{{ __('Quantity') }}</th><th class="text-end">{{ __('Saleable') }}</th><th class="text-end">{{ __('Rejected / Rework / Scrap') }}</th></tr></thead><tbody>@foreach($returns as $row)<tr><td>{{ str($row->reason_code)->replace('_', ' ')->title() }}</td><td class="text-end">{{ $row->return_count }}</td><td class="text-end">{{ $numbers->format($row->returned_quantity) }}</td><td class="text-end">{{ $numbers->format($row->saleable_quantity) }}</td><td class="text-end">{{ $numbers->format($row->rejected_quantity) }}</td></tr>@endforeach</tbody></table></div></div>
    <div class="card mt-3"><div class="card-header"><h6 class="mb-0">{{ __('Customer / Item Return Analysis') }}</h6></div><div class="table-responsive"><table class="table table-sm mb-0"><thead><tr><th>{{ __('Customer') }}</th><th>{{ __('Item') }}</th><th>{{ __('Reason') }}</th><th>{{ __('Disposition') }}</th><th class="text-end">{{ __('Quantity') }}</th></tr></thead><tbody>@foreach($returnAnalysis as $row)<tr><td>{{ $row->customer_doc_num }} / {{ $row->customer_name }}</td><td>{{ $row->product_doc_num }} / {{ $row->product_name }}</td><td>{{ str($row->reason_code)->replace('_', ' ')->title() }}</td><td>{{ $row->quality_disposition ?: '—' }}</td><td class="text-end">{{ $numbers->format($row->returned_quantity) }}</td></tr>@endforeach</tbody></table></div></div>
</div>
@endsection
