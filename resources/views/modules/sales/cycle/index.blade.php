...
@php
    ...
    $canCreate = in_array($kind, ['sales_requests', 'sales_orders', 'customer_receipts']);
    $states = match($kind) {
...
<div class="card erp-datatable-card">
    <x-admin.crud-index-toolbar :title="$titles[$kind]" :add-route="$canCreate ? route($prefix.'.create') : null" :add-permission="$kind.'.create'" />
