...
<div class="container-fluid">
    ...
    <div class="row g-3 mb-3">
        @foreach([['positions', $totals['positions']], ['quantity', $numbers->format($totals['quantity'])], ['book_value', $numbers->format($totals['book_value']).' '.$currencyCode], ['unvalued_quantity', $numbers->format($totals['unvalued_quantity'])], ['negative_positions', $totals['negative_positions']]] as [$key, $value])
    ...
    <div class="card d-print-none">
        ...
        <div class="card-body">
            <form method="GET" action="{{ route('admin.inventory.reports.valuation') }}" class="row g-3 align-items-end mb-3">
                ...
                <div class="col-md-3"><label class="form-label">{{ __('inventory_accounting.valuation_report.store') }}</label><x-forms.select variant="local" name="branch_store_id" required><option value="">{{ __('inventory_accounting.valuation_report.select') }}</option>@foreach($stores as $store)<option value="{{ $store->getKey() }}" @selected($selectedStore?->is($store))>{{ $store->branch?->name }} — {{ $store->name }}</option>@endforeach</x-forms.select></div>
