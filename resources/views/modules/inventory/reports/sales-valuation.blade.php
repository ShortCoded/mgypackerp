...
<div class="container-fluid">
    ...
    <form class="card card-body mb-3 d-print-none" method="GET" action="{{ route('admin.inventory.sales-valuation') }}">
        <div class="row g-3 align-items-end">
            ...
            <div class="col-md-3"><label class="form-label">@lang('stock_balance_inquiry.filters.store')</label><x-forms.select name="branch_store_uuid" class="js-select2-ajax" data-url="{{ route('admin.inventory.select2.branch-stores', ['scope' => 'company']) }}" data-allow-clear="true"><option value="">@lang('stock_balance_inquiry.options.all')</option>@foreach($options['stores'] as $store)<option value="{{ $store->public_uuid }}" @selected(($filters['branch_store_uuid'] ?? null) === $store->public_uuid)>{{ $store->branch?->name }} — {{ $store->name }}</option>@endforeach</x-forms.select></div>
            <div class="col-md-3"><label class="form-label">@lang('stock_balance_inquiry.filters.hall')</label><x-forms.select name="branch_hall_uuid" class="js-select2-ajax" data-url="{{ route('admin.inventory.select2.branch-halls', ['scope' => 'company']) }}" data-allow-clear="true"><option value="">@lang('stock_balance_inquiry.options.all')</option>@foreach($options['halls'] as $hall)<option value="{{ $hall->public_uuid }}" @selected(($filters['branch_hall_uuid'] ?? null) === $hall->public_uuid)>{{ $hall->branch?->name }} — {{ $hall->name }}</option>@endforeach</x-forms.select></div>
            ...
            <div class="col-md-3"><label class="form-label">@lang('stock_balance_inquiry.filters.item_classification')</label><x-forms.select variant="local" name="item_classification"><option value="">@lang('stock_balance_inquiry.options.all')</option>@foreach(\Modules\Core\Models\Product::stockableItemClassifications() as $classification)<option value="{{ $classification }}" @selected(($filters['item_classification'] ?? null) === $classification)>@lang('products.classifications.'.$classification)</option>@endforeach</x-forms.select></div>
            <div class="col-md-3"><label class="form-label">@lang('stock_balance_inquiry.filters.quantity_state')</label><x-forms.select variant="local" name="quantity_state"><option value="">@lang('stock_balance_inquiry.options.all')</option><option value="positive" @selected(($filters['quantity_state'] ?? null) === 'positive')>@lang('stock_balance_inquiry.quantity_states.positive')</option><option value="negative" @selected(($filters['quantity_state'] ?? null) === 'negative')>@lang('stock_balance_inquiry.quantity_states.negative')</option></x-forms.select></div>
    ...
    <div class="card mb-4"><div class="table-responsive"><table class="table table-bordered align-middle mb-0">
        <thead><tr>
            ...
            <th>@lang('stock_balance_inquiry.columns.hall')</th>
