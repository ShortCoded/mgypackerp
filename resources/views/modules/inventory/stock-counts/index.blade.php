...
<div class="card mb-3"><div class="card-header"><h5 class="mb-0">{{ __('Create Count Snapshot') }}</h5></div><div class="card-body"><form method="POST" action="{{ route('admin.inventory.stock-counts.store') }}" class="row g-3 align-items-end">@csrf
    ...
    <div class="col-md-3"><label class="form-label">{{ __('Stock status') }}</label><x-forms.select class="form-select" name="stock_status"><option value="">{{ __('All') }}</option>@foreach($stockStatuses as $status)<option value="{{ $status }}">{{ __(str($status)->replace('_', ' ')->title()->toString()) }}</option>@endforeach</x-forms.select></div>
