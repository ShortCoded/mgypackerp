...
<div class="card">
    <div class="card-header d-flex justify-content-between"><div><h5 class="mb-1">{{ $record->doc_num }}</h5><span class="badge bg-secondary">{{ __('inventory.movements.statuses.'.$record->status) }}</span></div>@can('inventory.stock_counts.print')<a class="btn btn-falcon-default btn-sm" href="{{ route('admin.inventory.stock-counts.print', $record) }}">{{ __('Print') }}</a>@endcan</div>
<form id="count-form" method="POST" action="{{ route('admin.inventory.stock-counts.record', $record) }}">@csrf<x-forms.line-item-cards />
    <div class="table-responsive"><table class="table table-sm mb-0"><thead><tr><th>{{ __('Product') }}</th><th>{{ __('Status') }}</th><th>{{ __('Batch') }}</th><th>{{ __('System') }}</th><th>{{ __('Physical') }}</th><th>{{ __('Variance reason') }}</th></tr></thead><tbody>@foreach($record->lines as $index => $line)<tr><td>{{ $line->product?->doc_num }} — {{ $line->product?->name }}<x-forms.input form="count-form" type="hidden" name="lines[{{ $index }}][line_id]" value="{{ $line->id }}" /></td><td>{{ __('inventory.movements.stock_statuses.'.$line->stock_status) }}</td><td>{{ $line->batch_lot }}</td><td>{{ $numbers->format($line->system_quantity) }}</td><td><x-forms.numeric-input form="count-form" class="form-control-sm" :scale="8" step="0.00000001" arrow-step="1" min="0" name="lines[{{ $index }}][physical_quantity]" :value="$line->physical_quantity" :disabled="$record->status !== 'draft'" required /></td><td><x-forms.input form="count-form" class="form-control form-control-sm" name="lines[{{ $index }}][variance_reason]" value="{{ $line->variance_reason }}" :disabled="$record->status !== 'draft'" /></td></tr>@endforeach</tbody></table></div>
</form>
