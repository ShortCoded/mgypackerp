...
<div class="card">
    <div class="card-header d-flex justify-content-between"><div><h5 class="mb-1">{{ $record->doc_num }}</h5><span class="badge bg-secondary">{{ $record->status }}</span></div>@can('inventory.stock_counts.print')<a class="btn btn-falcon-default btn-sm" href="{{ route('admin.inventory.stock-counts.print', $record) }}">{{ __('Print') }}</a>@endcan</div>
