...
@endphp
<div class="card mb-3">
    <div class="card-header d-flex justify-content-between"><div><h5 class="mb-1">{{ $record->doc_num }}</h5><span class="badge bg-secondary">{{ __(str($record->status)->replace('_', ' ')->title()->toString()) }}</span></div>@can('inventory.documents.print')<a class="btn btn-falcon-default btn-sm" href="{{ route('admin.inventory.documents.print', $record) }}">{{ __('Print') }}</a>@endcan</div>
    <div class="card-body"><div class="row g-2"><div class="col-md-3"><strong>{{ __('Type') }}:</strong> {{ __(str($record->document_type)->replace('_', ' ')->title()->toString()) }}</div><div class="col-md-3"><strong>{{ __('Date') }}:</strong> {{ $record->document_date?->toDateString() }}</div><div class="col-md-3"><strong>{{ __('Source') }}:</strong> {{ $record->branchStore?->name }}</div><div class="col-md-3"><strong>{{ __('Destination') }}:</strong> {{ $record->destinationBranchStore?->name }}</div><div class="col-md-3"><strong>{{ __('Source document') }}:</strong> {{ $record->source_doc_num }}</div>@if($record->productionOrder)<div class="col-md-3"><strong>{{ __('Production order') }}:</strong> <a href="{{ route('admin.production.work-orders.show', $record->productionOrder) }}">{{ $record->productionOrder->doc_num }}</a></div>@endif @if($record->productionRun)<div class="col-md-3"><strong>{{ __('Production run') }}:</strong> <a href="{{ route('admin.production.runs.show', $record->productionRun) }}">{{ $record->productionRun->run_number }}</a></div>@endif</div></div>
