...
<tr data-procurement-line>
    <td><x-forms.input type="hidden" name="lines[{{ $index }}][public_id]" value="{{ $line['public_id'] ?? '' }}" /><span data-line-number>{{ is_numeric($index) ? $index + 1 : '__NUMBER__' }}</span></td>
    <td class="line-card-wide"><x-forms.select class="form-select js-select2-ajax js-procurement-product" name="lines[{{ $index }}][product_doc_num]" data-url="{{ route('admin.purchases.select2.products') }}" data-placeholder="{{ __('Select item') }}" data-template="product-image" data-allow-clear="true" required>@if(!empty($line['product_doc_num']))<option value="{{ $line['product_doc_num'] }}" selected>{{ $product ? $product->doc_num.' / '.$product->name : ($line['product_text'] ?? $line['product_doc_num']) }}</option>@endif</x-forms.select></td>
    <td><x-forms.select class="form-select js-select2-local js-procurement-unit" name="lines[{{ $index }}][unit_doc_num]" data-placeholder="{{ __('Unit') }}" required>@foreach($unitOptions as $option)<option value="{{ $option['id'] }}" @selected($option['id'] === $unitDocNum)>{{ $option['text'] }}</option>@endforeach @if($unitDocNum && !collect($unitOptions)->contains('id', $unitDocNum))<option selected value="{{ $unitDocNum }}">{{ $line['unit_text'] ?? $unitDocNum }}</option>@endif</x-forms.select></td>
    <td><x-forms.numeric-input name="lines[{{ $index }}][requested_quantity]" :scale="8" min="0.00000001" step="0.00000001" arrow-step="1" :value="$line['requested_quantity'] ?? ''" required /></td>
    ...
    <td class="line-card-full"><x-forms.input class="form-control" name="lines[{{ $index }}][notes]" value="{{ $line['notes'] ?? '' }}" />
        @if(!empty($line['source_doc_num']))
            ...
            @foreach(['source_type', 'source_doc_num', 'source_line_reference', 'required_date', 'specification'] as $field)<x-forms.input type="hidden" name="lines[{{ $index }}][{{ $field }}]" value="{{ $line[$field] ?? '' }}" />@endforeach
        @endif
    </td>
