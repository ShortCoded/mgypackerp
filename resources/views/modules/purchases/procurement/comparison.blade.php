    ...
    <div class="card-body p-0"><div class="table-responsive procurement-lines-scroll"><table class="table table-sm table-hover align-middle mb-0 procurement-lines-table"><thead class="bg-100"><tr><th>{{ __('Item') }}</th><th>{{ __('Supplier') }}</th><th class="text-end">{{ __('Offered quantity') }}</th>@if($showPrices)<th>{{ __('Currency') }}</th><th class="text-end">{{ __('Unit price') }}</th><th class="text-end">{{ __('Discount') }}</th><th class="text-end">{{ __('Tax') }}</th><th class="text-end">{{ __('Freight') }}</th><th class="text-end">{{ __('Line total') }}</th><th class="text-end">{{ __('Landed base / unit') }}</th>@endif<th>{{ __('Delivery') }}</th><th>{{ __('Lead time') }}</th><th>{{ __('Payment terms') }}</th></tr></thead><tbody>
        @foreach($lines->groupBy('request_for_quotation_line_id') as $offers)
            ...
            @foreach($offers as $line)
                ...
                <tr class="{{ $showPrices && abs($normalized - $lowest) < 0.0001 ? 'table-success' : '' }}"><td>{{ $line->rfqLine?->product?->name }}</td><td>{{ $line->quotation?->supplier?->name }}</td><td class="text-end" dir="ltr">{{ $numbers->format($line->offered_quantity) }}</td>@if($showPrices)<td>{{ $line->quotation?->currency?->name ?: '—' }}</td><td class="text-end" dir="ltr">{{ $numbers->format($line->unit_price) }}</td><td class="text-end" dir="ltr">{{ $numbers->format($line->discount_amount) }}</td><td class="text-end" dir="ltr">{{ $numbers->format($line->tax_amount) }}</td><td class="text-end" dir="ltr">{{ $numbers->format($line->quotation->freight_amount) }}</td><td class="text-end fw-semibold" dir="ltr">{{ $numbers->format($line->line_total) }}</td><td class="text-end" dir="ltr">{{ $numbers->format($normalized) }}</td>@endif<td>{{ $line->delivery_date?->format('Y-m-d') ?: '—' }}</td><td>{{ $line->quotation?->lead_time_days ?? '—' }}</td><td>{{ $line->quotation?->payment_terms ?: '—' }}</td></tr>
            @endforeach
        @endforeach
    </tbody></table></div></div>
</div>
