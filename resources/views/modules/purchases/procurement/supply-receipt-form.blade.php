...
<form method="POST" action="{{ $draft ? route('admin.purchases.goods-receipt-notes.update', $draft->doc_num) : route('admin.purchases.goods-receipt-notes.store', $record->doc_num) }}">
    ...
    <div class="card mb-3">
        ...
        <div class="card-body">
            ...
            <div class="row g-3">
                <div class="col-12 col-md-3"><label class="form-label">{{ __('Supply Order') }}</label><input class="form-control" value="{{ $record->doc_num }}" dir="ltr" readonly></div>
                <div class="col-12 col-md-3"><label class="form-label">{{ __('Purchase Order') }}</label><input class="form-control" value="{{ $record->purchaseOrder?->doc_num }}" dir="ltr" readonly></div>
                <div class="col-12 col-md-3"><label class="form-label">{{ __('Supplier') }}</label><input class="form-control" value="{{ $record->supplier?->name }}" readonly></div>
                <div class="col-12 col-md-3"><label class="form-label">{{ __('Destination') }}</label><input class="form-control" value="{{ $record->branchStore?->name }}" readonly></div>
                <div class="col-12 col-md-3"><label class="form-label" for="document_date">{{ __('Receipt date') }}</label><input class="form-control js-date-picker" type="text" data-date-format="{{ $dates->jsDateFormat() }}" data-locale="{{ app()->getLocale() }}" dir="ltr" id="document_date" name="document_date" value="{{ old('document_date', $dates->formatDate($draft?->document_date ?? now())) }}" required></div>
                <div class="col-12 col-md-3"><label class="form-label" for="supplier_delivery_note">{{ __('Supplier delivery note') }}</label><input class="form-control" id="supplier_delivery_note" name="supplier_delivery_note" value="{{ old('supplier_delivery_note', $draft?->supplier_delivery_note) }}"></div>
                <div class="col-12 col-md-3"><label class="form-label" for="supplier_delivery_date">{{ __('Supplier delivery date') }}</label><input class="form-control js-date-picker" type="text" data-date-format="{{ $dates->jsDateFormat() }}" data-locale="{{ app()->getLocale() }}" dir="ltr" id="supplier_delivery_date" name="supplier_delivery_date" value="{{ old('supplier_delivery_date', $dates->formatDate($draft?->reference_date, '')) }}"></div>
                <div class="col-12 col-md-3"><label class="form-label" for="notes">{{ __('Notes') }}</label><input class="form-control" id="notes" name="notes" value="{{ old('notes', $draft?->notes) }}"></div>
            </div>
        </div>
    </div>
                ...
                <div class="card border shadow-none h-100">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row g-3 align-items-end">
                            <div class="col-6 col-lg-2"><label class="form-label">{{ __('Supply ordered') }}</label><input class="form-control" value="{{ $numbers->format($line->ordered_quantity) }}" dir="ltr" readonly></div>
                            <div class="col-6 col-lg-2"><label class="form-label">{{ __('Previously received') }}</label><input class="form-control" value="{{ $numbers->format($previouslyReceived) }}" dir="ltr" readonly></div>
                            <div class="col-6 col-lg-2"><label class="form-label">{{ __('Remaining') }}</label><input class="form-control" value="{{ $numbers->format($remaining) }}" dir="ltr" readonly></div>
                            <div class="col-6 col-lg-2"><label class="form-label">{{ __('QC') }}</label><input class="form-control" value="{{ $line->product?->requiresIncomingInspection() ? __('Inspection required') : __('Auto accepted') }}" readonly></div>
                            ...
                            @if($line->product?->tracks_expiry || $draftLine?->expiry_date)
                                ...
                                <div class="col-6 col-lg-2"><label class="form-label">{{ __('Expiry date') }}</label><input class="form-control js-date-picker" type="text" data-date-format="{{ $dates->jsDateFormat() }}" data-locale="{{ app()->getLocale() }}" dir="ltr" name="lines[{{ $index }}][expiry_date]" value="{{ old('lines.'.$index.'.expiry_date', $dates->formatDate($draftLine?->expiry_date, '')) }}"></div>
                            @endif
                            <div class="col-12 col-lg"><label class="form-label">{{ __('Notes') }}</label><input class="form-control" name="lines[{{ $index }}][notes]" value="{{ old('lines.'.$index.'.notes', $draftLine?->notes) }}"></div>
                        </div>
                    </div>
                </div>
            </div>
