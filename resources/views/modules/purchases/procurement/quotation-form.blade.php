...
<form method="POST" action="{{ $draft ? route('admin.purchases.supplier-quotation-entry.update', $draft->doc_num) : route('admin.purchases.supplier-quotation-entry.store-source', [$sourceType, $record->doc_num]) }}">
    ...
    <div class="card mb-3">@include('modules.purchases.procurement.partials.form-toolbar', ['toolbarTitle' => __('Supplier Quotation for :document', ['document' => $record->doc_num]), 'toolbarPermission' => 'purchases.supplier_quotation_entry', 'toolbarRoute' => 'supplier-quotation-entry'])<div class="card-body">
        ...
        <div class="row g-3">
            ...
            <div class="col-md-3"><label class="form-label">{{ __('Supplier reference') }}</label><x-forms.input class="form-control" name="supplier_reference" value="{{ old('supplier_reference', $draft?->supplier_reference) }}" /></div>
            <div class="col-md-3"><label class="form-label">{{ __('Quotation date') }}</label><x-forms.date-input class="form-control js-date-picker" type="text" data-date-format="{{ $dates->jsDateFormat() }}" data-locale="{{ app()->getLocale() }}" dir="ltr" name="quotation_date" value="{{ old('quotation_date', $dates->formatDate($draft?->quotation_date ?? now())) }}" required /></div>
            <div class="col-md-3"><label class="form-label">{{ __('Valid until') }}</label><x-forms.date-input class="form-control js-date-picker" type="text" data-date-format="{{ $dates->jsDateFormat() }}" data-locale="{{ app()->getLocale() }}" dir="ltr" name="valid_until" value="{{ old('valid_until', $dates->formatDate($draft?->valid_until, '')) }}" /></div>
            <div class="col-md-3"><label class="form-label">{{ __('Lead time (days)') }}</label><x-forms.input class="form-control" type="number" min="0" name="lead_time_days" value="{{ old('lead_time_days', $draft?->lead_time_days) }}" /></div>
            <div class="col-md-3"><label class="form-label">{{ __('Payment terms') }}</label><x-forms.input class="form-control" name="payment_terms" value="{{ old('payment_terms', $draft?->payment_terms) }}" /></div>
