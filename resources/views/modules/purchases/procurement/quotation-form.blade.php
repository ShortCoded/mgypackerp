...
<form method="POST" action="{{ $draft ? route('admin.purchases.supplier-quotation-entry.update', $draft->doc_num) : route('admin.purchases.supplier-quotation-entry.store', $record->doc_num) }}">
    ...
    <div class="card mb-3">@include('modules.purchases.procurement.partials.form-toolbar', ['toolbarTitle' => __('Supplier Quotation for :document', ['document' => $record->doc_num]), 'toolbarPermission' => 'purchases.supplier_quotation_entry', 'toolbarRoute' => 'supplier-quotation-entry'])<div class="card-body">
        ...
        <div class="row g-3">
            <div class="col-md-4"><label class="form-label">{{ __('Supplier') }}</label><select class="form-select js-select2-ajax" name="supplier_doc_num" required data-url="{{ route('admin.purchases.select2.suppliers', ['rfq' => $record->doc_num]) }}" data-placeholder="{{ __('Select') }}"><option value="">{{ __('Select') }}</option>@foreach($selectedSuppliers as $supplier)<option value="{{ $supplier->doc_num }}" selected>{{ $supplier->doc_num }} / {{ $supplier->name }}</option>@endforeach</select></div>
    ...
    <div class="card mb-3"><div class="card-body p-0"><div class="table-responsive procurement-lines-scroll"><table class="table table-sm align-middle mb-0 procurement-lines-table"><thead class="bg-100"><tr><th>#</th><th>{{ __('Item') }}</th><th class="text-end">{{ __('Requested') }}</th><th>{{ __('Offered quantity') }}</th><th>{{ __('Unit price') }}</th><th>{{ __('Discount') }}</th><th>{{ __('Tax %') }}</th><th>{{ __('Delivery date') }}</th><th>{{ __('Notes') }}</th></tr></thead><tbody>
        @foreach($record->lines as $index => $line)
            @php($draftLine = $draft?->lines->firstWhere('request_for_quotation_line_id', $line->id))
            <tr><td>{{ $index + 1 }}</td><td>{{ $line->product?->name }}<input type="hidden" name="lines[{{ $index }}][rfq_line_public_id]" value="{{ $line->public_id }}"></td><td class="text-end" dir="ltr">{{ $line->quantity }}</td><td><x-forms.numeric-input name="lines[{{ $index }}][offered_quantity]" :scale="8" min="0.00000001" step="0.00000001" :value="old('lines.'.$index.'.offered_quantity', $draftLine?->offered_quantity ?? $line->quantity)" required /></td><td><x-forms.numeric-input name="lines[{{ $index }}][unit_price]" :scale="4" min="0" step="0.0001" :value="old('lines.'.$index.'.unit_price', $draftLine?->unit_price)" required /></td><td><x-forms.numeric-input name="lines[{{ $index }}][discount_amount]" :value="old('lines.'.$index.'.discount_amount', $draftLine?->discount_amount ?? 0)" :scale="4" min="0" step="0.0001" /></td><td><x-forms.numeric-input name="lines[{{ $index }}][tax_rate]" :value="old('lines.'.$index.'.tax_rate', $draftLine?->tax_rate ?? 0)" :scale="4" min="0" max="100" step="0.0001" /></td><td><input class="form-control js-date-picker" type="text" data-date-format="{{ $dates->jsDateFormat() }}" data-locale="{{ app()->getLocale() }}" dir="ltr" name="lines[{{ $index }}][delivery_date]" value="{{ old('lines.'.$index.'.delivery_date', $dates->formatDate($draftLine?->delivery_date, '')) }}"></td><td><input class="form-control" name="lines[{{ $index }}][notes]" value="{{ old('lines.'.$index.'.notes', $draftLine?->notes) }}"></td></tr>
        @endforeach
