...
@php
    $resource = 'suppliers';
    $routePrefix = 'admin.purchases.suppliers';
...
@section('content')
    @can('suppliers.document_number_settings.update')
        <div class="card mb-3">
            <div class="card-header py-2">
                <button class="btn btn-link text-decoration-none p-0 w-100 text-start d-flex align-items-center justify-content-between"
                    ...
                    data-bs-target="#suppliers-document-number-settings"
                    aria-expanded="false"
                    aria-controls="suppliers-document-number-settings">
                    <span class="fw-semibold">{{ __('common.document_number_settings.title') }}</span>
                    <span class="fas fa-chevron-down fs-11"></span>
                </button>
            </div>
            <div class="collapse" id="suppliers-document-number-settings">
                <div class="card-body">
                    ...
                    <form class="js-business-document-number-settings-form"
                        ...
                        <div class="row g-3 align-items-end">
                            <div class="col-md-6 col-lg-4">
                                <label class="form-label" for="suppliers-document-prefix">{{ __('common.document_number_settings.prefix') }}</label>
                                <x-forms.input class="form-control" id="suppliers-document-prefix" name="prefix" type="text" maxlength="20" value="{{ $documentNumberSettings['prefix'] ?? '' }}" />
                                <div class="invalid-feedback d-block" data-error-for="prefix"></div>
                            </div>
                            <div class="col-md-3 col-lg-2">
                                <label class="form-label" for="suppliers-document-padding">{{ __('common.document_number_settings.padding') }}</label>
                                <x-forms.input class="form-control" id="suppliers-document-padding" name="padding" type="number" min="0" max="10" step="1" value="{{ $documentNumberSettings['padding'] ?? 0 }}" required />
        ...
        <div class="card-header">
            <div class="row flex-between-center">
                ...
                <div class="col-6 col-sm-auto ms-auto text-end ps-0 d-flex justify-content-end align-items-center gap-2">
                    @can('suppliers.view_trashed')
                        <div class="d-flex align-items-center gap-2">
                            <label class="form-label mb-0 text-700 fs-10" for="suppliers_trash_filter">{{ __('business_partners.trash.filter_label') }}</label>
                            <x-forms.select class="form-select form-select-sm w-auto js-business-trash-filter" id="suppliers_trash_filter" aria-label="{{ __('business_partners.trash.filter_label') }}">
                    ...
                    @endcan
                    <x-buttons.add-record :href="route($routePrefix.'.create')" permission="suppliers.create" />
                </div>
            </div>
        </div>
            ...
            <div class="falcon-data-table">
                <div class="erp-datatable-wrapper">
                    <div class="erp-datatable-scroll">
                        <table id="suppliers-table" class="table table-sm table-hover mb-0 data-table erp-datatable align-middle js-business-partner-table"
                            data-url="{{ route($routePrefix.'.data') }}"
                            data-table-name="suppliers">
