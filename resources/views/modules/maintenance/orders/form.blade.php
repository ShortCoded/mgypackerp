...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.maintenance.orders.store') }}" class="card">@csrf
        ...
        <div class="card-body"><div class="row g-3">
            ...
            <div class="col-md-4"><label class="form-label">{{ __('maintenance.fields.supplier') }}</label><x-forms.select variant="ajax" :url="route('admin.maintenance.select2', ['lookup' => 'suppliers'])" placeholder="—" class="form-select" name="supplier_id"><option value="">—</option>@foreach ($suppliers as $supplier)<option value="{{ $supplier->id }}">{{ $supplier->doc_num }} — {{ $supplier->name }}</option>@endforeach</x-forms.select></div>
