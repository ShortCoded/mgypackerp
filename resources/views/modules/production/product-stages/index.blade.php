...
@section('content')
    ...
    <div class="card mb-3">
        ...
        <div class="card-body">
            <div class="row g-3 align-items-end"><div class="col-md-8"><label class="form-label">{{ __('production_execution.fields.product') }}</label><x-forms.select id="product-route-selector" variant="ajax" :url="route('admin.production.product-stages.select2.products')" :placeholder="__('common.placeholders.select')"><option value=""></option></x-forms.select></div><div class="col-md-4"><button class="btn btn-primary" type="button" data-navigate-select="#product-route-selector">{{ __('production_execution.actions.configure_route') }}</button></div></div>
        </div>
    </div>
