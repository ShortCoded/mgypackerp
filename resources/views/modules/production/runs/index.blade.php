...
<div class="card"><div class="card-header"><h5 class="mb-0">{{ __('Production Runs') }}</h5></div><div class="table-responsive"><table class="table table-sm table-hover align-middle mb-0"><thead><tr><th>{{ __('Run') }}</th><th>{{ __('Order') }}</th><th>{{ __('Product') }}</th><th>{{ __('Schedule') }}</th><th>{{ __('Machine / Mold') }}</th><th>{{ __('Planned') }}</th><th>{{ __('Good') }}</th><th>{{ __('Status') }}</th></tr></thead><tbody>@forelse($records as $record)<tr><td><a href="{{ route('admin.production.runs.show', $record) }}">{{ $record->run_number }}</a></td><td>{{ $record->order?->doc_num }}</td><td>{{ $record->product?->name }}</td><td>{{ $record->planned_start_at?->format('Y-m-d H:i') }} → {{ $record->planned_end_at?->format('Y-m-d H:i') }}</td><td>{{ $record->machine?->name }} / {{ $record->mold?->name }}</td><td>{{ $record->planned_base_quantity }}</td><td>{{ $record->good_base_quantity }}</td><td><span class="badge bg-secondary">{{ __(str($record->status)->replace('_', ' ')->title()->toString()) }}</span></td></tr>@empty<tr><td colspan="8" class="text-center py-5 text-500">{{ __('No production runs found.') }}</td></tr>@endforelse</tbody></table></div>@if($records->hasPages())<div class="card-footer">{{ $records->links() }}</div>@endif</div>
@endsection
