...
@section('content')
    ...
    @can('costing.overhead_allocation_run.create')
        <div class="card mb-3"><div class="card-header"><h2 class="h6 mb-0">{{ __('overhead_allocations.new_preview') }}</h2></div><div class="card-body">
            <form method="POST" action="{{ route('admin.costing.overhead-allocation-run.preview') }}" class="row g-3 align-items-end">
                @csrf
                <div class="col-md-6"><label class="form-label">{{ __('overhead_allocations.rule') }}</label><x-forms.select name="rule_public_id" variant="local" required><option value=""></option>@foreach($rules as $rule)<option value="{{ $rule->public_id }}" @selected(old('rule_public_id') === $rule->public_id)>{{ $rule->doc_num }} / {{ $rule->name }}</option>@endforeach</x-forms.select></div>
                <div class="col-md-2"><label class="form-label">{{ __('overhead_allocations.from_date') }}</label><x-forms.date-input name="from_date" :value="old('from_date', $period->from_date->toDateString())" required /></div>
                <div class="col-md-2"><label class="form-label">{{ __('overhead_allocations.to_date') }}</label><x-forms.date-input name="to_date" :value="old('to_date', $period->to_date->toDateString())" required /></div>
    ...
    @if($selectedRun)
        <div class="row g-3 mb-3">
            ...
            <div class="col-6 col-lg-3"><div class="card h-100"><div class="card-body py-2"><div class="text-600 fs-11">{{ __('overhead_allocations.unused_capacity_cost') }}</div><strong dir="ltr">{{ $numbers->format($selectedRun->unusedCapacityCost()) }}</strong>@if($selectedRun->unusedCapacityReason())<div class="small text-600">{{ __('overhead_allocations.unused_capacity_reasons.'.$selectedRun->unusedCapacityReason()) }}</div>@endif</div></div></div>
        </div>
        <div class="card mb-3"><div class="card-header d-flex flex-wrap justify-content-between gap-2"><div><strong dir="ltr">{{ $selectedRun->doc_num }}</strong> — {{ $selectedRun->rule->name }} <span class="badge badge-subtle-secondary">{{ __('overhead_allocations.statuses.'.$selectedRun->status) }}</span></div><div>{{ __('overhead_allocations.basis_used') }}: {{ __('overhead_allocations.basis_options.'.$selectedRun->basis_used) }}@if($selectedRun->actual_capacity !== null) · {{ __('overhead_allocations.actual_capacity') }}: <span dir="ltr">{{ $numbers->format($selectedRun->actual_capacity) }}</span>@endif @if($selectedRun->rule->normal_capacity_hours !== null) · {{ __('overhead_allocations.normal_capacity_hours') }}: <span dir="ltr">{{ $numbers->format($selectedRun->rule->normal_capacity_hours) }}</span>@endif @if($selectedRun->utilization_percent !== null) · {{ __('overhead_allocations.utilization') }}: <span dir="ltr">{{ $numbers->format($selectedRun->utilization_percent) }}%</span>@endif</div></div>
            @if($selectedRun->fallback_reason)<div class="alert alert-warning m-3 mb-0">{{ $selectedRun->fallback_reason }}</div>@endif
            <div class="card-body p-0"><div class="table-responsive"><table class="table table-sm table-striped mb-0"><thead><tr><th>{{ __('overhead_allocations.production_run') }}</th><th>{{ __('overhead_allocations.work_order') }}</th><th>{{ __('overhead_allocations.product') }}</th><th>{{ __('overhead_allocations.machine_hours') }}</th><th>{{ __('overhead_allocations.labor_hours') }}</th><th>{{ __('overhead_allocations.direct_material_cost') }}</th><th>{{ __('overhead_allocations.basis_value') }}</th><th>{{ __('overhead_allocations.allocation_percent') }}</th><th>{{ __('overhead_allocations.allocated_amount') }}</th></tr></thead><tbody>@foreach($selectedRun->lines as $line)<tr><td>@if($line->productionRun)<a href="{{ route('admin.production.runs.show', $line->productionRun) }}">{{ $line->productionRun->run_number }}</a>@endif</td><td>{{ $line->productionRun?->order?->doc_num }}</td><td>{{ $line->productionRun?->product?->name }}</td><td dir="ltr">{{ $line->machine_hours }}</td><td dir="ltr">{{ $line->labor_hours }}</td><td dir="ltr">{{ $numbers->format($line->direct_material_cost) }}</td><td dir="ltr">{{ $numbers->format($line->basis_value) }}</td><td dir="ltr">{{ $numbers->format($line->allocation_percent) }}%</td><td dir="ltr">{{ $numbers->format($line->allocated_amount) }}</td></tr>@endforeach</tbody></table></div></div>
            <div class="card-footer d-flex flex-wrap gap-2">
                ...
                @if($selectedRun->status === 'posted') @can('costing.overhead_allocation_run.reverse')<form method="POST" action="{{ route('admin.costing.overhead-allocation-run.reverse', $selectedRun->public_id) }}" class="d-flex gap-2">@csrf<x-forms.input name="reason" :placeholder="__('overhead_allocations.reversal_reason')" required /><button class="btn btn-outline-danger" type="submit">{{ __('overhead_allocations.reverse') }}</button></form>@endcan @endif
            </div>
        </div>
        <div class="card mb-3"><div class="card-header"><h2 class="h6 mb-0">{{ __('overhead_allocations.source_details') }}</h2></div><div class="card-body p-0"><div class="table-responsive"><table class="table table-sm mb-0"><thead><tr><th>{{ __('overhead_allocations.source_entry') }}</th><th>{{ __('overhead_allocations.account') }}</th><th>{{ __('overhead_allocations.amount') }}</th></tr></thead><tbody>@foreach($selectedRun->sources as $source)<tr><td>@if($source->journalEntryLine?->journalEntry)<a href="{{ route('admin.accounting.journal-entries.show', $source->journalEntryLine->journalEntry) }}">{{ $source->journalEntryLine->journalEntry->doc_num }}</a>@endif</td><td>{{ $source->account?->codeNameLabel() }}</td><td dir="ltr">{{ $numbers->format($source->source_amount) }}</td></tr>@endforeach</tbody></table></div></div></div>
    @endif
