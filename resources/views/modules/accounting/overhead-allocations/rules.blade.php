        ...
        <div class="card mb-3">
            ...
            <div class="card-body">
                <form method="POST" action="{{ route('admin.costing.overhead-allocation-rules.store') }}" class="row g-3">
                    ...
                    <div class="col-md-4"><x-forms.label :label="__('overhead_allocations.source_cost_center')" required /><x-forms.select id="overhead-source-cost-center" name="source_cost_center_id" variant="ajax" :url="route('admin.costing.overhead-allocation-rules.select2.cost-centers')" :allow-clear="false" required><option value=""></option>@if($selectedSourceCostCenter)<option value="{{ $selectedSourceCostCenter->id }}" selected>{{ $selectedSourceCostCenter->codeNameLabel() }}</option>@endif</x-forms.select></div>
                    <div class="col-md-4"><x-forms.label :label="__('overhead_allocations.source_accounts')" required /><x-forms.select name="source_account_ids[]" variant="ajax" :url="route('admin.costing.overhead-allocation-rules.select2.source-accounts')" multiple data-depends-on="#overhead-source-cost-center" data-dependent-param="source_cost_center_id" data-disable-when-dependency-empty="true" :allow-clear="false" required>@foreach($selectedSourceAccounts as $account)<option value="{{ $account->id }}" selected>{{ $account->codeNameLabel() }}</option>@endforeach</x-forms.select></div>
                    <div class="col-md-4"><label class="form-label">{{ __('overhead_allocations.target_cost_centers') }}</label><x-forms.select name="target_cost_center_ids[]" variant="ajax" :url="route('admin.costing.overhead-allocation-rules.select2.cost-centers')" multiple :placeholder="__('overhead_allocations.all_target_centers')">@foreach($selectedTargetCostCenters as $center)<option value="{{ $center->id }}" selected>{{ $center->codeNameLabel() }}</option>@endforeach</x-forms.select></div>
                    <div class="col-md-3"><x-forms.label :label="__('overhead_allocations.basis')" required /><x-forms.select name="basis" variant="local" :allow-clear="false" required>@foreach(['machine_hours', 'labor_hours', 'direct_material_cost'] as $basis)<option value="{{ $basis }}" @selected(old('basis', 'machine_hours') === $basis)>{{ __('overhead_allocations.basis_options.'.$basis) }}</option>@endforeach</x-forms.select></div>
                    <div class="col-md-3"><label class="form-label">{{ __('overhead_allocations.fallback_basis') }}</label><x-forms.select name="fallback_basis" variant="local"><option value="">{{ __('overhead_allocations.no_fallback') }}</option><option value="direct_material_cost" @selected(old('fallback_basis') === 'direct_material_cost')>{{ __('overhead_allocations.basis_options.direct_material_cost') }}</option></x-forms.select></div>
                    <div class="col-md-3"><x-forms.label :label="__('overhead_allocations.cost_behavior')" required /><x-forms.select name="cost_behavior" variant="local" :allow-clear="false" required>@foreach(['variable', 'fixed'] as $behavior)<option value="{{ $behavior }}" @selected(old('cost_behavior', 'variable') === $behavior)>{{ __('overhead_allocations.behavior_options.'.$behavior) }}</option>@endforeach</x-forms.select></div>
