        ...
        <div class="card mb-3">
            ...
            <div class="card-body">
                <form method="POST" action="{{ route('admin.costing.overhead-allocation-rules.store') }}" class="row g-3">
                    @csrf
                    <div class="col-md-6"><label class="form-label">{{ __('overhead_allocations.name') }}</label><x-forms.input name="name" :value="old('name')" required /></div>
                    ...
                    <div class="col-md-4"><label class="form-label">{{ __('overhead_allocations.source_cost_center') }}</label><x-forms.select name="source_cost_center_id" variant="local" required><option value=""></option>@foreach($costCenters as $center)<option value="{{ $center->id }}" @selected((string) old('source_cost_center_id') === (string) $center->id)>{{ $center->codeNameLabel() }}</option>@endforeach</x-forms.select></div>
                    <div class="col-md-4"><label class="form-label">{{ __('overhead_allocations.source_accounts') }}</label><x-forms.select name="source_account_ids[]" variant="local" multiple required>@foreach($accounts as $account)<option value="{{ $account->id }}" @selected(in_array((string) $account->id, array_map('strval', old('source_account_ids', [])), true))>{{ $account->codeNameLabel() }} — {{ $account->costCenters->map->codeNameLabel()->implode(', ') }}</option>@endforeach</x-forms.select></div>
                    <div class="col-md-4"><label class="form-label">{{ __('overhead_allocations.target_cost_centers') }}</label><x-forms.select name="target_cost_center_ids[]" variant="local" multiple :placeholder="__('overhead_allocations.all_target_centers')">@foreach($costCenters as $center)<option value="{{ $center->id }}" @selected(in_array((string) $center->id, array_map('strval', old('target_cost_center_ids', [])), true))>{{ $center->codeNameLabel() }}</option>@endforeach</x-forms.select></div>
                    <div class="col-md-3"><label class="form-label">{{ __('overhead_allocations.basis') }}</label><x-forms.select name="basis" required>@foreach(['machine_hours', 'labor_hours', 'direct_material_cost'] as $basis)<option value="{{ $basis }}" @selected(old('basis', 'machine_hours') === $basis)>{{ __('overhead_allocations.basis_options.'.$basis) }}</option>@endforeach</x-forms.select></div>
                    ...
                    <div class="col-md-3"><label class="form-label">{{ __('overhead_allocations.cost_behavior') }}</label><x-forms.select name="cost_behavior" required>@foreach(['variable', 'fixed'] as $behavior)<option value="{{ $behavior }}" @selected(old('cost_behavior', 'variable') === $behavior)>{{ __('overhead_allocations.behavior_options.'.$behavior) }}</option>@endforeach</x-forms.select></div>
                    <div class="col-md-3"><label class="form-label">{{ __('overhead_allocations.normal_capacity_hours') }}</label><x-forms.input type="number" min="0.00000001" step="0.00000001" name="normal_capacity_hours" :value="old('normal_capacity_hours')" /></div>
                    <div class="col-md-3"><label class="form-label">{{ __('overhead_allocations.effective_from') }}</label><x-forms.date-input name="effective_from" :value="old('effective_from', $period->from_date->toDateString())" required /></div>
