        ...
        <div class="py-2 border dropdown-menu dropdown-menu-end">
            @if ($isTrashed)
                ...
                @if ($canRestore)
                    ...
                    @endif
                    <button type="button" class="dropdown-item text-success js-restore-record" data-doc-num="{{ $record->doc_num }}" data-record-name="{{ $record->name }}" data-restore-url="{{ route($definition->route('restore'), $record->doc_num) }}" data-item-lookup-restore-url="{{ route($definition->route('restore'), $record->doc_num) }}">
                        {{ __('item_lookups.trash.restore') }}
                    </button>
                @endif
            @else
                @if ($canView)
                ...
                @can($definition->permission('delete'))
                    <div class="dropdown-divider"></div>
                    <button type="button" class="dropdown-item text-danger js-delete-record" data-doc-num="{{ $record->doc_num }}" data-record-name="{{ $record->name }}" data-delete-url="{{ route($definition->route('destroy'), $record->doc_num) }}" data-item-lookup-delete-url="{{ route($definition->route('destroy'), $record->doc_num) }}">
