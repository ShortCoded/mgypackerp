            ...
            <div class="card mb-3">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('admin.hr.leave-types.store') }}" class="row g-3">
                        ...
                        <div class="col-md-4"><label class="form-label">{{ __('hr_leave_types.fields.payment_status') }}</label><x-forms.select variant="local" name="payment_status" :allow-clear="false" required>@foreach (['paid', 'unpaid'] as $status)<option value="{{ $status }}" @selected(old('payment_status', 'paid') === $status)>{{ __('hr_leave_types.payment_statuses.'.$status) }}</option>@endforeach</x-forms.select></div>
                        ...
                        <div class="col-md-3"><label class="form-label">{{ __('hr_leave_types.fields.status') }}</label><x-forms.select variant="local" name="status" :allow-clear="false">@foreach (['active', 'inactive'] as $status)<option value="{{ $status }}">{{ __('hr_leave_types.statuses.'.$status) }}</option>@endforeach</x-forms.select></div>
                ...
                <div class="col-12 col-xl-6">
                    <div class="card h-100"><div class="card-body">
                        ...
                        <form method="POST" action="{{ route('admin.hr.leave-types.update', $leaveType) }}" class="row g-2">
                            ...
                            <div class="col-6"><x-forms.select variant="local" name="payment_status" :allow-clear="false" :disabled="! auth()->user()?->can('hr.leave_types.update')">@foreach (['paid', 'unpaid'] as $status)<option value="{{ $status }}" @selected(data_get($leaveType->metadata, 'payment_status', 'paid') === $status)>{{ __('hr_leave_types.payment_statuses.'.$status) }}</option>@endforeach</x-forms.select></div>
                            ...
                            <div class="col-5"><x-forms.select variant="local" name="status" :allow-clear="false" :disabled="! auth()->user()?->can('hr.leave_types.update')">@foreach (['active', 'inactive'] as $status)<option value="{{ $status }}" @selected($leaveType->status === $status)>{{ __('hr_leave_types.statuses.'.$status) }}</option>@endforeach</x-forms.select></div>
