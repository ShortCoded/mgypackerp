...
@section('content')
    <div class="container-fluid px-0 px-sm-3">
        ...
        @can('hr.payroll_attendance_policies.manage')
            <div class="card mb-3"><div class="card-header"><h5 class="mb-0">{{ __('hr_payroll_policies.create') }}</h5></div><div class="card-body">
                <form method="POST" action="{{ route('admin.hr.payroll-attendance-policies.store') }}" class="row g-3">
                    @csrf
                    <div class="col-md-4"><label class="form-label">{{ __('hr_payroll_policies.fields.branch') }}</label><x-forms.select variant="ajax" name="branch_doc_num" :url="route('admin.select2.branches')" :placeholder="__('hr_payroll_policies.company_scope')">@if ($canCreateCompanyPolicy)<option value="">{{ __('hr_payroll_policies.company_scope') }}</option>@endif @if($selectedPolicyBranch)<option value="{{ $selectedPolicyBranch->doc_num }}" selected>{{ $selectedPolicyBranch->name }}</option>@endif</x-forms.select></div>
                    <div class="col-md-4"><label class="form-label">{{ __('hr_payroll_policies.fields.effective_from') }}</label><x-forms.date-input name="effective_from" :value="old('effective_from', now()->toDateString())" required /></div>
                    <div class="col-md-4"><label class="form-label">{{ __('hr_payroll_policies.fields.deduction_payroll_item_code') }}</label><x-forms.select variant="local" name="deduction_payroll_item_code"><option value=""></option>@foreach ($payrollItems as $item)<option value="{{ $item->code }}" @selected(old('deduction_payroll_item_code') === $item->code)>{{ $item->name }} ({{ $item->code }})</option>@endforeach</x-forms.select></div>
        ...
        <div class="table-responsive"><table class="table table-striped align-middle"><thead><tr><th>{{ __('hr_payroll_policies.scope') }}</th><th>{{ __('hr_payroll_policies.fields.effective_from') }}</th><th>{{ __('hr_payroll_policies.fields.effective_to') }}</th><th>{{ __('hr_payroll_policies.fields.deduction_payroll_item_code') }}</th></tr></thead><tbody>
            @forelse ($policies as $policy)<tr><td>{{ $policy->branch?->name ?? __('hr_payroll_policies.company_scope') }}</td><td>{{ $dates->formatDate($policy->effective_from, '') }}</td><td>{{ $dates->formatDate($policy->effective_to, '—') }}</td><td>{{ $policy->deduction_payroll_item_code ?? '—' }}</td></tr>@empty<tr><td colspan="4" class="text-center text-muted py-4">{{ __('hr_payroll_policies.messages.empty') }}</td></tr>@endforelse
        </tbody></table></div>
