        ...
        <div class="py-2 border dropdown-menu dropdown-menu-end">
            @if ($isTrashed)
                ...
                @if ($canRestore)
                    ...
                    @endif
                    <button type="button" class="dropdown-item text-success js-restore-record" data-doc-num="{{ $record->doc_num }}" data-record-name="{{ $record->name }}" data-hr-foundation-restore-url="{{ route($definition->route('restore'), $record->doc_num) }}">
                        {{ __('hr.trash.restore') }}
                    </button>
                @endif
            @else
                ...
                @can($definition->permission('delete'))
                    <div class="dropdown-divider"></div>
                    <button type="button" class="dropdown-item text-danger js-delete-record" data-doc-num="{{ $record->doc_num }}" data-record-name="{{ $record->name }}" data-hr-foundation-delete-url="{{ route($definition->route('destroy'), $record->doc_num) }}">
