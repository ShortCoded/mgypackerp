...
<body onload="window.print()">
    ...
    <table class="meta"><tr><td>{{ __('hr_payroll_reports.columns.period') }}<br><strong dir="ltr">{{ $dates->formatDate($payslip->period_start, '—') }} — {{ $dates->formatDate($payslip->period_end, '—') }}</strong></td><td>{{ __('hr_payroll_reports.columns.branch') }}<br><strong>{{ $payslip->branch_name ?: '—' }}</strong></td><td>{{ __('hr_payroll_reports.columns.status') }}<br><strong>{{ __('hr_payroll.status.'.$payslip->status) }}</strong></td></tr></table>
