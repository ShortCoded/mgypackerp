...
<div class="row g-3 mb-3">
    @foreach (['period' => $payslip->period_start.' — '.$payslip->period_end, 'branch' => $payslip->branch_name ?: '—', 'gross' => $numbers->format($payslip->gross_amount), 'deductions' => $numbers->format($payslip->deduction_amount), 'net' => $numbers->format($payslip->net_amount), 'status' => __('hr_payroll.status.'.$payslip->status)] as $key => $value)
...
<div class="card mb-3"><div class="card-header"><h5 class="mb-0">{{ __('hr_payroll_reports.payslip.items') }}</h5></div><div class="table-responsive"><table class="table table-sm mb-0">
    ...
    <tbody>@foreach($items as $item)<tr><td><span dir="ltr">{{ $item->code }}</span>@if($item->name)<span class="d-block small text-muted">{{ $item->name }}</span>@endif</td><td>{{ __('hr_payroll_reports.directions.'.$item->direction) }}</td><td>{{ $item->source_type ?: '—' }}@if($item->source_snapshot !== [])<span class="d-block small text-muted" dir="ltr">{{ collect($item->source_snapshot)->map(fn ($value, $key) => $key.': '.(is_array($value) ? json_encode($value, JSON_UNESCAPED_UNICODE) : $value))->implode(' | ') }}</span>@endif</td><td class="text-end" dir="ltr">{{ $numbers->format($item->amount) }}</td></tr>@endforeach</tbody>
</table></div></div>
<div class="row g-3">
    <div class="col-lg-4"><div class="card h-100"><div class="card-header"><h5 class="mb-0">{{ __('hr_payroll_reports.payslip.payroll_snapshot') }}</h5></div><div class="card-body"><dl class="row mb-0">
        ...
        <dt class="col-7">{{ __('hr_payroll_reports.columns.gross') }}</dt><dd class="col-5 text-end" dir="ltr">{{ $numbers->format(data_get($input, 'gross', 0)) }}</dd>
        <dt class="col-7">{{ __('hr_payroll_reports.columns.deductions') }}</dt><dd class="col-5 text-end" dir="ltr">{{ $numbers->format(data_get($input, 'deductions', 0)) }}</dd>
        <dt class="col-7">{{ __('hr_payroll_reports.columns.net') }}</dt><dd class="col-5 text-end" dir="ltr">{{ $numbers->format(data_get($input, 'net', 0)) }}</dd>
    </dl></div></div></div>
